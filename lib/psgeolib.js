...
function PsgeoLib() {
    ...
    function getDivingEnvironmentSubActivities(includeUnknown = false) {
        var result = ["divingEnvironment-pool",
                      "divingEnvironment-Spring",
                      "divingEnvironment-Lake",
                      "divingEnvironment-River",
                      "divingEnvironment-Sea",
                      "divingEnvironment-Sandpit",
                      "divingEnvironment-Stonequarry",
                      "divingEnvironment-Mine",
                      "divingEnvironment-Cave",
                      "divingEnvironment-Sump",
                      "divingEnvironment-Harbour",
                      "divingEnvironment-Industrial",
                      "divingEnvironment-Other"];
        if (includeUnknown) result.push("DivingEnvironment-Unknown");
        return(result);
    }

    function getDivingTaskSubActivities(includeUnknown = false) {
        var result = ["DivingTask-RecreationalOpenWater",
                      "DivingTask-Ice",
                      "DivingTask-Mine",
                      "DivingTask-Cave",
                      "DivingTask-Sump",
                      "DivingTask-Wreck",
                      "DivingTask-Wreckpenetration",
                      "DivingTask-UnderwaterStructurePenetration",
                      "DivingTask-PhotoVideo",
                      "DivingTask-FishingHunting",
                      "DivingTask-GuidingAndInstruction",
                      "DivingTask-Biology",
                      "DivingTask-Archaeology",
                      "DivingTask-Installation",
                      "DivingTask-InspectionRepair",
                      "DivingTask-Recovery",
                      "DivingTask-Rescue",
                      "Diving-Other"];
        if (includeUnknown) result.push("DivingTask-Unknown");
        return(result);
    }
    ...
    // Swimmming can be done in a waterbody or in some other place (pallomeri!)
    ...
    function getDefaultFilter(db,activity) {
        switch (activity) {
        case 'Skiing': return getDefaultFilterSkiing(db);
        case 'Caving': return getDefaultFilterCaving(db);
        case 'Urbanex': return getDefaultFilterUrbanex(db);
        case 'Climbing': return getDefaultFilterClimbing(db);
        case 'Biking': return getDefaultFilterBiking(db);
        case 'Swimming': return getDefaultFilterSwimming(db);
        case 'Diving': return getDefaultFilterDiving(db);
        case 'Flying': return getDefaultFilterFlying(db);
        case 'Sauna': return getDefaultFilterSauna(db);
        default: {
            psgeoDebug("Default filter " + activity + " is not defined in psgeolib.js getDefaultFilter()")
	    return db.filterTrue();
            }
        }
    }
    ...
    function getDefaultFilterDiving(db) {
        return(db.filterMatchActivity(jsonSchemaActivityDiving));
    }
    ...
    function round(value,decimals) {
        let scale = Math.pow(10, decimals || 0 )
        return(Math.round(scale * value) / scale);
    }

    function floor(value,decimals) {
        let scale = Math.pow(10, decimals || 0 )
        return(Math.floor(scale * value) / scale);
    }

    function numberWithSpaces(x,lang) {
        // Group three digits and use decimal dot or comma
        //
        // /pattern/g, " " Replace each matching pattern with a space
        // \B      matches at any position between two characters. 
        // ?=      the match must be followed by whatever is within the parentheses
        // (\d{3}) three digits
        // +       match one or more of the preceding token
        // (?!\d)  one or zero non-digits
        let xWithSpaces = x.toString().replace(/\B(?=(\d{3})+(?!\d))/g, " ");

        if (Number.isInteger(x)) {
            return xWithSpaces
        } else { // decimal
            // if (language === 'fi' || language === 'sv' ) return xWithSpaces.replace(/\./,',')
            // i18n belongs to lang, not lib. Hence:
            let decimalSeparator = lang.getText('decimalSeparator');
            return xWithSpaces.replace(/\./,decimalSeparator)
        }
    }
    ...
    //
    // Google Maps style tweaking functions ---------------------------------------------------
    //

    function reCenterCustomControl(id) {
        let uiElement = document.getElementById(id)
        let controlWidth = uiElement.offsetWidth
        uiElement.style.left = (window.innerWidth - controlWidth)/2 + 'px'
    } 
    ...
    return({
        ...
        getDivingEnvironmentSubActivities: getDivingEnvironmentSubActivities,
        getDivingTaskSubActivities: getDivingTaskSubActivities,
        ...
        reCenterCustomControl: reCenterCustomControl,
        ...
        debugPrintObject: debugPrintObject,
