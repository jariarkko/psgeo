...
function PsgeoDB(lib,
    ...
    const subActivityGroupsWithUnknown = [
        ...
        //
        // Diving: There are environments and tasks.
        // Both should be marked as unkwnon if not specified.
        //
        ...
        { activities: ["Diving"],
          subActivitySets: [
              { condition: undefined,
                negation: undefined,
                subActivities: lib.getDivingEnvironmentSubActivities(true),
                unknown: "Waterbody-Unknown",
                warning: "Diving, but no waterbody type specified" },
              { condition: undefined,
                negation: undefined,
                subActivities: lib.getDivingTaskSubActivities(true),
                unknown: "DivingTask-Unknown",
                warning: "Diving, but no task specified" }]},
    ...
    function normalizeItemFields(place,source) {
        ...
        //    - video field (v) exists but is empty 
        ...
        // Video field exists but is empty?
        if (place.v !== undefined && place.v === "") {
            if (warncallback !== undefined) {
                warncallback("V",
                             "Item " +
                             place.n +
                             " v field is set to an empty string (if there is no video, leave the field out)",
                             source);
            }
            place.d3 = undefined;
        }
    ...
    function removeDuplicates(list,source) {
        // source is only needed in warncallback error message!
        var newList = [];
        var newListStrings = [];
        var nDuplicates = 0;
        var lastDuplicate = "";
    ...
    function filterMatchVideo(needsVideo) {
        return({ op: "videoMatch", param1: needsVideo});
    }
    
    function filterMatchModel(needsModel) {
        return({ op: "modelMatch", param1: needsModel});
    }
    ...
    function setFilter(newFilter) {
        // psgeoDebug("db.setFilter: filter:")
        // psgeoDebug(lib.debugPrintObject(newFilter)) // Useful debugging but fails 'make test'
        filter = newFilter
    }
    ...
    function matchFilter(entry,condition) {
        ...
        case "modelMatch":
            return(condition.param1 === false ||
                   (entry.d3 !== undefined));
        case "videoMatch":
            if (condition.param1 === false) return true
            for (const article of entry.rl) {
                if (article.v !== undefined) return true
            }
            return false;
        case "articleMatch":
            return(condition.param1 === false ||
    ...
    var dbThis = {
        ...
        filterMatchVideo: filterMatchVideo,
