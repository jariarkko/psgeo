...
function PsgeoDB(lib,listin,source) {
    ...
    function itemDescriptionGenericSubActivities(lang,
        ...
        // sentences       A boolean. True if we should make each separate sub activity set a
        ...
        if (!isboolean(sentences)) throw("sentences is not a boolean");
        if (!isboolean(useComma)) throw("useCommat is not a boolean");
        ...
        for (var k = 0; k < subActivitySets.length; k++) {
            ...
            if (sentences || k + 1 == subActivitySets.length) {
                result += ".";
            } else {
                var separator = useComma ? "," : lang.getText("and");
