...
function PsgeoDB(lib,listin,source) {
    ...
    function itemDescriptionGenericSubActivities(lang,
        ...
        for (var k = 0; k < subActivitySets.length; k++) {
            ...
            var activityIndex = indexOfActivityPresent(item,activities);
    ...
    function indexOfActivityPresent(item,activities) {
        if (!isitem(item)) throw ("Not a dataset item");
        if (!isarray(activities)) throw ("Not an array");
        for (var i = 0; i < activities.length; i++) {
            if (hasItemActivity(item,activities[i])) return(i);
        }
        return(undefined);
    }
