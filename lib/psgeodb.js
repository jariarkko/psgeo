...
function PsgeoDB(lib,listin,source) {
    ...
    function itemStrongerThanAnother(mainitem,otheritem) {
        // Map wins
        if (mainitem.m !== undefined && otheritem.m === undefined) return(true);
        // Picture wins
        if (mainitem.i !== undefined && otheritem.i === undefined) return(true);
        // More articles wins
        if (mainitem.rl.length > otheritem.rl.length) return(true);
    ...
    function itemDescriptionAuxRLItem(lang,item,rli,useHtml,conserveSpace) {
        ...
        if (getReadingListItemURL(rli) !== undefined) {
            ...
            //debug("item description point 1 title = " +  getReadingListItemTitle(rli));
            var pretext = "";
            if (getReadingListItemTitle(rli) === undefined ||
                getReadingListItemTitle(rli) === "") {
                debug("item description point 2");
                pretext = lang.capitalize(lang.publicationText()) + ": ";
            } else {
                //debug("item description point 3");
                pretext =
                    lang.capitalize(lang.articleText()) + ": " +
                    lang.capitalize(getReadingListItemTitle(rli)) + " " +
                    lang.inPublicationText() + " ";
            }
            //debug("item description point 4 pretext = " + pretext);
    ...
    function describeItem(lang,item,useHtml = true,conserveSpace = false) {
        ...
        if (descr != undefined) {
            ...
            if (!conserveSpace) {
                var alternates = getItemAlternativeDescriptionsLang(item,lang.getLanguage());
                for (var da = 0; da < alternates.length; da++) {
                    ...
                    html += descriptionText(lang,altdescr,useHtml,conserveSpace);
                }
            }
        }
