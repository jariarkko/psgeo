...
function PsgeoDB(lib,listin,source) {
    ...
    function descriptionTitle(lang,text) {
        if (!islang(lang)) throw ("Not a language object");
        return(lib.htmlParagraph(lib.htmlBold(text)));
    }

    function descriptionImage(lang,url) {
        if (!islang(lang)) throw ("Not a language object");
        return(lib.htmlImage(url,'width="140"'));
    }

    function descriptionText(lang,text) {
        if (!islang(lang)) throw ("Not a language object");
        return(lib.htmlParagraph(text));
    }

    function descriptionTextLine(lang,text) {
        if (!islang(lang)) throw ("Not a language object");
        return(text + lib.htmlBreak());
    }

    function descriptionTextAndLink(lang,text,link,url) {
        if (!islang(lang)) throw ("Not a language object");
        return(text + lib.htmlLink(link,url,true));
    }

    function descriptionTextAndLinkAndText(lang,text1,link,url,text2) {
        if (!islang(lang)) throw ("Not a language object");
        return(text1 + lib.htmlLink(link,url,true) + text2);
    }

    function descriptionCoordinates(lang,lat,lon,includetext) {
        ...
        var text = descriptionTextAndLink(lang,textHeader,textCoords,urlCoords);
        return(text);
    }

    function psgeoFindImageUrl(item,rl) {
        if (!isitem(item)) throw ("Not a dataset item");
    ...
    function psgeoActivityText(lang,list) {
        if (!islang(lang)) throw ("Not a language object");
        var authorsandyear = itemDescriptionAuxRLItemAuthorsAndYear(lang,rli);
    ...
    function itemDescriptionAuxRLItemAuthorsAndYear(lang,rli) {
        if (!islang(lang)) throw ("Not a language object");
    ...
    function itemDescriptionTextActivities(lang,item) {
        if (!islang(lang)) throw ("Not a language object");
        if (!isitem(item)) throw ("Not a dataset item");
        ...
        if (movementissimple && otheractivitiesissimple) {
            ...
            return(descriptionText(lang,descr));
        }
    }

    function itemDescriptionTextMainCave(lang,item) {
        if (!islang(lang)) throw ("Not a language object");
        if (!isitem(item)) throw ("Not a dataset item");
    ...
    function itemDescriptionTextSideCaves(lang,item) {
        if (!islang(lang)) throw ("Not a language object");
        if (!isitem(item)) throw ("Not a dataset item");
        var htmlInside = "";
        ...
        for (var i = 0; i < sidecaves.length; i++) {
            ...
            thisHtml += ": ";
            thisHtml += sidecave;
            if (i+1 < sidecaves.length) thisHtml = descriptionTextLine(lang,thisHtml);
            htmlInside += thisHtml;
        }
        if (htmlInside.length > 0) {
            return(descriptionText(lang,htmlInside));
        } else {
            return("");
        }
    }

    function itemDescriptionAlternativeNames(lang,item) {
        if (!islang(lang)) throw ("Not a language object");
        if (!isitem(item)) throw ("Not a dataset item");
    ...
    function itemDescriptionAlternativeCoordinates(lang,item) {
        if (!islang(lang)) throw ("Not a language object");
        if (!isitem(item)) throw ("Not a dataset item");
    ...
    function describeItem(lang,item) {
        if (!islang(lang)) throw ("Not a language object");
        if (!isitem(item)) throw ("Not a dataset item");
        var rl = getItemReadingList(item);
        var html = "";
        html +=  descriptionTitle(lang,getItemName(item));
        if (hasItemReadingListImageUrl(item)) {
            var imageUrl = psgeoFindImageUrl(item,rl);
            html +=  descriptionImage(lang,imageUrl);
        }
        var descr = getItemDescriptionLang(item,lang.getLanguage());
        if (descr != undefined) {
            html += descriptionText(lang,descr);
        }
        if (getItemFuzzy(item)) {
            html += descriptionText(lang,lang.capitalize(lang.locationSecretText()));
        } else {
            var coordstext = descriptionCoordinates(lang,
                                                    getItemLat(item),
                                                    getItemLon(item),
                                                    true);
            coordstext += itemDescriptionAlternativeCoordinates(lang,item);
        ...
        if (getItemLength(item) !== undefined) {
            ...
            lengthtext += itemDescriptionAlternativeNames(lang,item);
        ...
        html += itemDescriptionTextActivities(lang,item);
        var htmlinside = "";
        for (var i = 0; i < rl.length; i++) {
            var thishtml = itemDescriptionAuxRLItem(lang,item,rl[i]);
            if (i+1 < rl.length) thishtml = descriptionTextLine(lang,thishtml);
            htmlinside += thishtml;
        }
        html += descriptionText(lang,htmlinside);
        html += itemDescriptionTextMainCave(lang,item);
        html += itemDescriptionTextSideCaves(lang,item);
        return(html);
    }
