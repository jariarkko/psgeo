...
function PsgeoDB(lib,listin,source) {
    ...
    var warnedAboutLuolakirjaDescriptionInName = false;
    ...
    function normalizeItemFields(place,source) {
        ...
        var id = findLuolakirjaId(place.n);
        if (id !== undefined) {
            if (warnedAboutLuolakirjaDescriptionInName) {
                warn("Dataset item " + place.n + " has a Suomen Luolat book identifier in its name, should be in description");
            }
            var idbrackets = "[" + id + "]";
            ...
            var lkdesc = findLuolakirjaDescription(place.n);
            if (lkdesc !== undefined) {
    ...
    function findLuolakirjaId(name) {
        //debug("findLuolakirjaId " + name);
        var position = name.indexOf("[");
        //debug("findLuolakirjaId pos=" + position.toString());
    ...
    function findLuolakirjaDescription(name) {
        ...
        var result  = name.substring(start,end);
        //debug("findLuolakirjaDescription " + result);
        return(result);
    }

    function addLuolakirjaCategories(item,type) {
        //debug("addLuolakirjaCategories "  + item.n + " type " + type);
        while (type.length > 0) {
            var found = false;
            for (var i = 0; i < luolaKirjaTypes.length; i++) {
                const knownType = luolaKirjaTypes[i];
                if (type.substring(0,knownType.letter.length) === knownType.letter) {
                    //debug("match " + knownType.letter);
            ...
            if (!found) {
                //debug("not recognised: " + type);
                warn("Unrecognised luolakirja category " + type);
                return;
            }
        }
    }
