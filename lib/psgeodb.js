...
function PsgeoDB(lib,listin,source) {
    ...
    function descriptionImage(lang,name,url,useHtml,conserveSpace) {
        ...
        if (!isboolean(conserveSpace)) throw("conserveSpace is not a boolean");
        if (useHtml) {
            var imageLink = descriptionNameToURL(lang,name,true,useHtml,conserveSpace);
    ...
    function descriptionNameToURL(lang,name,imageUrl,useHtml,conserveSpace) {
        if (!islang(lang)) throw ("Not a language object in descriptionTextLine");
        if (!isstring(name)) throw ("Name is not a string");
        if (!isboolean(imageUrl)) throw("imageUrl is not a boolean");
        if (!isboolean(useHtml)) throw("useHtml is not a boolean");
        if (!isboolean(conserveSpace)) throw("conserveSpace is not a boolean");
        if (imageUrl) {
            return("?i=" + encodeURI(name));
        } else {
    ...
    function itemDescriptionTextMainCave(lang,item,useHtml,conserveSpace) {
        ...
        if (hasItemMainCave(item)) {
            ...
            return(descriptionTextAndLink(lang,
                                          ...
                                          descriptionNameToURL(lang,getItemMainCave(item),false,useHtml,conserveSpace),
                                          useHtml,
                                          conserveSpace));
        } else {
            return("");
        ...
        if (conserveSpace) {
            ...
            for (var i = 0; i < sidecaves.length; i++) {
                ...
                thisHtml += descriptionLink(lang,
                                            ...
                                            descriptionNameToURL(lang,sidecave,false,useHtml,conserveSpace),
                                            useHtml,
                                            conserveSpace);
    ...
    function describeItem(lang,
        ...
        if (hasItemReadingListImageUrl(item)) {
            ...
            html +=  descriptionImage(lang,getItemName(item),imageUrl,useHtml,conserveSpace);
        }
