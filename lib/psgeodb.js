...
function PsgeoDB(lib,listin,source) {
    ...
    function itemDescriptionGenericSubActivities(lang,
        ...
        //                 "and" depending on the setting for "useComma" parameter.
        // - useComma      A boolean. True if we should use comma to separate multiple sub
        ...
        for (var k = 0; k < subActivitySets.length; k++) {
            ...
            if (!isboolean(subActivitySet.sentence)) throw("sentences is not a boolean");
            if (!isboolean(subActivitySet.useComma)) throw("useCommat is not a boolean");
        ...
        for (var k = 0; k < subActivitySets.length; k++) {
            var subActivitySet = subActivitySets[k];
            var items = getItemMultiSubActivitiesWithinSet(item,activities,subActivitySet.subActivities);
            //debug("  printing subActivitySet " + JSON.stringify(subActivitySet.headerTextId));
            //debug("  itemTextId = " + subActivitySet.itemTextId);
            if (subActivitySet.headerTextId !== undefined) {
                result += lang.capitalize(lang.getText(subActivitySet.headerTextId)) + " ";
            }
            ...
            if (subActivitySet.sentence || k + 1 == subActivitySets.length) {
                result += ".";
            } else if (subActivitySet.useComma) {
                result += ",";
            } else {
