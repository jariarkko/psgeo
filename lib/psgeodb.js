...
function PsgeoDB(lib,listin) {

    // Private data
    
    var list = normalizeDataset(listin);
    ...
    function normalizeItem(place) {
        ...
        if  (place.type !== undefined &&
             place.type === "Feature") {
            return(normalizeItemGeoJSON(place));
        }
        ...
        return(normalizeItemFields(place));
    }

    function normalizeItemGeoJSON(place) {
    ...
    function normalizeItemFields(place) {
        ...
        if (isarray(dataset)) {
            ...
            if (dataset.type !== undefined &&
                ...
                for (i = 0; i < dataset.features.length; i++) {
                    var normalized = normalizeItem(dataset.features[i]);
    ...
    function addPlaces(dataset) {
        var places = normalizeDataset(dataset);
	var i;
	for (i = 0; i < places.length; i++) {
	    list.push(normalizeItem(places[i]));
	}
    }

    function addPlacesWithFilter(dataset,extraFilter) {
        var places = normalizeDataset(dataset);
	var i;
	for (i = 0; i < places.length; i++) {
            var place = places[i];
	    if (matchFilter(place,extraFilter)) {
	        list.push(normalizeItem(place));
            }
	}
    }
