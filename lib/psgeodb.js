...
function PsgeoDB(lib,listin,source) {
    ...
    const finnishCaveTypes = [
        ...
        {
            letter: "A",
            classification: ["Rock","Morphology-Crystallization"]
        },
        {
            letter: "B",
        ...
        {
            letter: "I",
            classification: ["Morphology-Other"]
        },
        {
            letter: "J",
            classification: ["Rock","Morphology-Erosion"]
    ...
    function normalizeItemFields(place,source) {
        ...
        if (id !== undefined) {
            ...
            if (type !== undefined) {
                ...
                moretexten += " Cave type class is " + type + ".";
                moretextfi += " Luolan tyyppi on " + type + ".";
                addFinnishCaveCategories(place,type);
            }

            //
            // See if we have other information in paranthesis
            //
            ...
            if (lkdesc !== undefined) {
                ...
                if (lib.isdigit(lkdesc[0])) {
                    prefixen = langen.dimensionsText() + " ";
                    prefixfi = langfi.dimensionsText() + " ";
                }
                moretexten += " " + langen.capitalize(prefixen + lkdesc) + ".";
                moretextfi += " " + langfi.capitalize(prefixfi + lkdesc) + ".";
            }
            addDescriptions(place,
                            { d: { en: langen.literatureCaveIdText() + " " + id + "." + moretexten,
                                   fi: langfi.literatureCaveIdText() + " " + id + "." + moretextfi },
                              s: source },
                            source);
        }
            ...
            if (sa.includes("Rock")) {
                ...
                if (!includesAny(sa,morphologySpecifics)) {
                    if (!warnedAboutLackOfMorphology) {
                        warn("Place " + place.n + " is caving in rock, but no morphology specified");
        ...
        if (hasItemActivity(place,"Swimming")) {
            ...
            if (sa.includes("Water")) {
                ...
                if (!includesAny(sa,waterbodySpecifics)) {
                    ...
                    //debug("adding waterbody-unknown to " + place.n);
    ...
    function rockTypeText(lang,prefix,list,manmade) {
        ...
        var text = lang.capitalize(lang.rockText() + ": ");
        for (var i = 0; i < list.length; i++) {
            if (i > 0) text += "/";
    ...
    function activityText(lang,list) {
        ...
        var text = lang.capitalize(lang.caveText() + "-" + lang.activityText() + ": ");
        for (var i = 0; i < list.length; i++) {
            if (i > 0) text += ", ";
    ...
    function itemDescriptionAuxRLItemPublication(lang,item,rli,authorsandyear,useHtml,conserveSpace) {
        ...
        if (getReadingListItemPublication(rli) !== undefined) {
            ...
            if (getReadingListItemTitle(rli) === undefined ||
                ...
                pretext = lang.capitalize(lang.publicationReferenceText()) + ": ";
            } else {
                pretext =
                    lang.capitalize(lang.publicationReferenceText()) + ": " +
                    lang.capitalize(getReadingListItemTitle(rli)) + ". "
                // lang.inPublicationText() + " ";
            }
            var pubitself = getReadingListItemPublication(rli);
            if (conserveSpace) {
                //debug("pubitself = "  + pubitself);
                //debug("shortens " + JSON.stringify(publicationNamesToShorten));
                for (var i = 0; i < publicationNamesToShorten.length; i++) {
                    const shorten = publicationNamesToShorten[i];
                    //debug("trying to shorten with " + JSON.stringify(shorten.pattern + " to " + shorten.shorter));
                    pubitself = pubitself.replace(shorten.pattern,shorten.shorter);
                    //debug("pubitself becomes " + pubitself);
                }
            }
    ...
    function itemDescriptionTextCavingActivities(lang,item,useHtml,conserveSpace) {
        ...
        if (!isitem(item)) throw ("Not a dataset item");
        var movementtypes = ["Basic","SRT","Swimming","Boating","Diving","Digging","Biking","Skiing"];
    ...
    function itemDescriptionSaunaType(lang,item,useHtml,conserveSpace) {
        if (!islang(lang)) throw ("Not a language object in itemDescriptionSaunaType");
        if (!isitem(item)) throw ("Not a dataset item");
        var placeTypes = ["Normal,Smoke,Steam"];
        var places = getItemSubActivitiesWithinSet(item,"Sauna",placeTypes);
        if (places.length == 0) {
            return("");
        }
        var usesDefaults = (places.length == 0 || (places.length == 1 && places[0] === "Normal"));
        if (usesDefaults && conserveSpace) {
            ...
            for (var i = 0; i < places.length; i++) {
                ...
                else if (i > 0 && i == places.length -1) result += " " + lang.andText() + " ";
                result += lang.saunaTypeValueText(places[i].toLowerCase());
            }
            result += ".";
            return(lang.capitalize(result));
        }
    }
    ...
    function itemDescriptionSwimmingPlace(lang,item,useHtml,conserveSpace) {
        ...
        if (!isitem(item)) throw ("Not a dataset item");
        var placeTypes = ["Outdoor","Indoor","Underground",
                          "Waterbody-Lake","Waterbody-River","Waterbody-Sea","Waterbody-Sandpit","Waterbody-Pool","Waterbody-Cave","Waterbody-Other",
                          "Beach","Nature",
                          "Thermal-Bath"];
        var materialTypes = ["Water","Ice","Balls"];
        ...
        if (usesDefaults && conserveSpace) {
            ...
            var result = lang.capitalize(lang.swimmingText()) + " ";
            for (var i = 0; i < places.length; i++) {
                if (i > 0 && i < places.length - 1) result += ", ";
                else if (i > 0 && i == places.length -1) result += " " + lang.andText() + " ";
                result += lang.swimmingPlaceValueText(places[i].toLowerCase());
            }
            if (places.length > 0 && materials.length > 0) result += ", ";
            for (var j = 0; j < materials.length; j++) {
                if (j > 0 && j < materials.length - 1) result += ", ";
                else if (j > 0 && j == materials.length -1) result += " " + lang.andText() + " ";
                result += lang.swimmingMaterialValueText(materials[j].toLowerCase());
            }
            result += ".";
            return(result);
        }
    ...
    function itemDescriptionSkiingPlaceAndMaterial(lang,item,useHtml,conserveSpace) {
        if (!islang(lang)) throw ("Not a language object in itemDescriptionSkiingPlaceAndMaterial");
        if (!isitem(item)) throw ("Not a dataset item");
        var placeTypes = ["Outdoor","Indoor"];
        var materialTypes = ["Snow","Sand","Rock","Ash","Asphalt","Grass","Mud","Plastic","Other-substances","Imported-snow"];
        var places = getItemSubActivitiesWithinSet(item,"Skiing",placeTypes);
        var indoor = places.includes("Indoor");
        var outdoor = places.includes("Outdoor");
        var materials = getItemSubActivitiesWithinSet(item,"Skiing",materialTypes);
        if (places.length == 0 && materials.length == 0) {
            return("");
        }
        var usesDefaults =
        ...
        if (usesDefaults && conserveSpace) {
            ...
            var result = lang.capitalize(lang.skiText()) + " ";
            if (indoor && outdoor) {
                result +=
                    lang.indoorText() + " " +
                    lang.andText() + " " +
                ...
                result += lang.indoorText() + " ";
            } else if (outdoor) {
                result += lang.outdoorText() + " ";
            }
            for (var j = 0; j < materials.length; j++) {
                if (j > 0 && j < materials.length - 1) result += ", ";
                else if (j > 0 && j == materials.length -1) result += " " + lang.andText() + " ";
                result += lang.skiMaterialValueText(materials[j].toLowerCase());
            }
            result += ".";
            return(result);
        }
    ...
    function itemDescriptionBikingTrackAndBikeType(lang,item,useHtml,conserveSpace) {
        ...
        if (!isitem(item)) throw ("Not a dataset item");
        var placeTypes = ["Track-City","Track-Road","Track-Trail","Track-Nature","Outdoor","Indoor","Underground"];
        var bikeTypes = ["Bike-City","Bike-Road","Bike-Mountain","Bike-Unicycle","Bike-Tandem","Bike-Tricycle","Bike-Other"];
        ...
        var result = lang.capitalize(lang.bikingText()) + " ";
        for (var i = 0; i < bikes.length; i++) {
            if (i > 0 && i < bikes.length - 1) result += ", ";
            else if (i > 0 && i== bikes.length -1) result += " " + lang.andText() + " ";
            result += lang.bikeTypeValueText(bikes[i].toLowerCase());
        }
        result = descriptionTextAdd(lang,result,lang.andText() + " ",useHtml,conserveSpace);
        for (var j = 0; j < places.length; j++) {
            if (j > 0 && j < places.length - 1) result += ", ";
            else if (j > 0 && j == places.length -1) result += " " + lang.andText() + " ";
            result += lang.bikeTrackValueText(places[j].toLowerCase());
        }
        result += ".";
        return(result);
    }
    ...
    function itemDescriptionUrbanExplorationTarget(lang,item,useHtml,conserveSpace) {
        ...
        if (!isitem(item)) throw ("Not a dataset item");
        var targets = getItemSubActivities(item,"Urban-exploration");
        if (targets.length == 0 ||
            (targets.length == 1 && targets[0] === "None")) {
            return("");
        }
        var result = lang.capitalize(lang.targetText()) + " ";
         //debug("targets = " + JSON.stringify(targets));
        for (var i = 0; i < targets.length; i++) {
            if (i > 0 && i < targets.length - 1) result += ", ";
            else if (i > 0 && i == targets.length -1) result += " " + lang.andText() + " ";
            result += lang.urbanexTargetValueText(targets[i].toLowerCase());
        }
        result += ".";
        return(result);
    }
    ...
    function itemDescriptionCavingCharacteristics(lang,item,useHtml,conserveSpace) {
        ...
        if (getItemLength(item) !== undefined) {
            ...
            lengthtext += ".";
            lengthtext = descriptionTextAdd(lang,lengthtext,rockTypeText(lang,rocktypeprefix,rocktype,manmade),useHtml,conserveSpace);
            lengthtext = descriptionTextAdd(lang,lengthtext,morphologyText(lang,morphologyprefix,morphology),useHtml,conserveSpace);
            if (hasItemAlternativeNames(item)) {
                lengthtext = descriptionTextAdd(lang,lengthtext,itemDescriptionAlternativeNames(lang,item,conserveSpace),useHtml,conserveSpace);
            }
            html +=
            ...
            lengthtext += ".";
            lengthtext = descriptionTextAdd(lang,
                                            lengthtext,
            ...
            html +=
                ...
                ": " +
                lengthtext;
        } else if (rocktype.length > 0 || morphology.length > 0) {
            var typetext = "";
            typetext = descriptionTextAdd(lang,
                                          typetext,
                                          rockTypeText(lang,rocktypeprefix,rocktype,manmade),
                                          useHtml,
                                          conserveSpace);
            typetext = descriptionTextAdd(lang,typetext,morphologyText(lang,morphologyprefix,morphology),useHtml,conserveSpace);
            if (hasItemAlternativeNames(item)) {
                typetext = descriptionTextAdd(lang,typetext,itemDescriptionAlternativeNames(lang,item),useHtml,conserveSpace);
            }
            html += typetext;
        }
    ...
    function itemDescriptionAlternativeNames(lang,item,conserveSpace) {
        ...
        //debug("alternative names: " + JSON.stringify(an));
        if (!conserveSpace && an.length > 0) {
            return( lang.capitalize(lang.alternativeNamesText()) +
                   ": " +
                    lang.listToText(an) +
                    ".");
        } else {
        ...
        for (var i = 0; i < rl.length; i++) {
            ...
            if (first) {
                htmlinside += descriptionTextLine(lang,
                                                  lang.capitalize(lang.moreToReadText() + ":"),
                                                  useHtml,
                                                  conserveSpace);
                first = false;
            }
    ...
    function describeItem(lang,
        ...
        characteristics = descriptionTextAdd(lang,
                                             characteristics,
                                             itemDescriptionCavingCharacteristics(lang,item,useHtml,conserveSpace),
                                             useHtml,conserveSpace);
        ...
        characteristics = descriptionTextAdd(lang,
                                             characteristics,
                                             itemDescriptionSkiingPlaceAndMaterial(lang,item,useHtml,conserveSpace),
                                             useHtml,conserveSpace);
