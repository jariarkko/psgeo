...
function PsgeoDB(lib,listin,source) {
    ...
    function itemDescriptionGenericSubActivities(lang,item,activity,subActivities,emptyItem,headerTextId,itemTextId,useHtml,conserveSpace) {
        // lang            The language object
        // item            The dataset item we are looking at
        // activity        The main activity we are looking at, e.g., "Skiing"
        ...
        if (!isitem(item)) throw ("Not a dataset item");
        var items = getItemSubActivitiesWithinSet(item,activity,subActivities);
        if (items.length == 0 ||
