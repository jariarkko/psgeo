...
function PsgeoDBText(lib) {
    ...
    const subActivityDescriptionTable = [
        ...
        //
        // Diving: here we have two sub-activity groups, the environment
        // tasks typical of the site.
        //
        ...
        { activities: ["Diving"],
          subActivitySets: [
              { subActivities: lib.getDivingEnvironmentSubActivities(true),
                ...
                omitEmpty: false,
                omitDefault: false,
                headerTextIds: ["diving"],
                colonAfterHeader: false,
                itemTextId: "divingEnvironment",
                useCommaAsItemSeparator: true,
                sentence: false,
    ...
    function describeItem(db,
        ...
        // psgeoDebug("psgeodbtext.js: describeItem: myPosition is: lat: " + myPosition.lat + " lon: " + myPosition.lng); // XXXXXX
        //
        ...
        if (!db.getItemFuzzy(item) && typeof myPosition !== 'undefined' && myPosition.lat !== null && myPosition.lng !== null) {
            ...
            // Long distances > 9999 m should be in km
            // We use 10 km as the limit in distance,
            // while length uses 2 km as transition point from m to km
            // (see function distanceInUnits)
            if (distance > 9999 ) {
                distance = lib.round(distance/1000,1) // one decimal
                distanceUnit = 'km'
            }
                
            html += lib.htmlParagraph(lang.getTextWithValues(
                "distanceAndBearing",
                false,
                [lib.numberWithSpaces(distance,lang),distanceUnit,bearing]
            ));
