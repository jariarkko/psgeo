...
function PsgeoLang(language) {
    ...
    var groundTypeValueTexts = [];			// non-rock type grounds 
    groundTypeValueTexts["en"] = {
	"ice": "ice",
	"snow": "snow",					// types of snow not yet included: https://en.wikipedia.org/wiki/Classifications_of_snow
	"sand": "sand",
	"gravel": "gravel",
	"moraine": "moraine",
	"rock": "rock",					// see rockTypeValueTexts for detailed classification
	"turf": "turf",
...
//	INTERNAL FUNCTIONS BEGIN
	...
	// {tab} is an array of arrays: it is an array of translation text arrays
	// {tab} name defines the controlledVocabulary beeing used (eg. rockTypeValueTexts=rockType list and so forth)
	// {tab}[value] returns a word of the controlled vocabulary, e.g. sandstoneTexts (english and finnish) from rockTypeValueTexts
	// Like this:
	//	tab			rockTypeValueTexts
	//	tab[value]		rockTypeValueTexts["limestone/dolomite"]
	//	tab[value][language]	rockTypeValueTexts["limestone/dolomite"]["fi"] = "kalkkikivi tai dolomiitti"
...
// METHODS BEGIN
	...
	function getTextCollectionItem(controlledVocabulary,tagText) {
		...
		// Replaces all: function {controlledVocabulary}ValueText(tagText)

		// Terminology: A collection (e.g. rockTypeValueTexts) determines a controlledVocabulary (a fixed set of words)
		//		These words can be called tags, because they are, in effect, metadata. Hence their variable name: tagText
		//		Each tagText is an array, which contains translations

		// controlledVocabularies of psgeo include (but are not limited to):
		...
		//	(activityJSON could come attached with controlledVocabulary files) but built-in will do for own use now
		...
		var source = controlledVocabulary + "ValueTexts";
...
return({
	getLanguage: 			getLanguage,		// returns string {language}
	getText:			getText,		// returns a string from translation array {textId}Texts or {textId}TextsShort in {language}
	getTextCollectionItem:		getTextCollectionItem,	// looks up the translation array {tagName} in array {controlledVocabulary} and returns a string in {language}
	getTextWithValues:		getTextWithValues,
