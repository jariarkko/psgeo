...
function dbAppendTrackList(trackId,trackDate) {
    ...
    request.onsuccess = function () {
        var db = request.result
        var transaction = db.transaction("trackList", IDBTransaction.READ_WRITE)
...
function dbAppendTrackRecord(trackId,trackDate,latlon) {
    ...
    request.onsuccess = function () {
        var db = request.result
        var transaction = db.transaction("recordedTracks", IDBTransaction.READ_WRITE)
...
function restoreTraceFromDb(lib,geolocation,trackStartDate,psgeoMap) {
...
function PsgeoLocation() {
    ...
    function getGeolocationButton(lang,
        ...
        async function geolocationPending() {
            ...
            trackDate = trackId.substring(0, 10)
            dbAppendTrackList(trackId,trackDate)
            psgeoDebug("Stored trackId: "+trackId+" on "+trackDate)
