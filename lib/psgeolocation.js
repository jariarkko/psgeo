...
function dbPutRecord(latlon) {
    ...
    request.onsuccess = function () {
        ...
        const isoHour = posixTime.toISOString().substring(11, 2) // HH
        psgeoDebug("New position. Timestamp "+posixTime+" Date "+isoDate+" Hour "+isoHour)

        // Store new position or update an existing one (until one minute has passed)
        var putPromise = objectStore.put({ positionRecordIndex: positionRecordIndex, date: isoDate, hour: isoHour, position: latlon })
...
function PsgeoLocation() {
    ...
    function getGeolocationButton(lang,
        ...
        function geolocationAchieved(position) {
            ...
            trace.push(LatLng)        // XXXX RECOVERY FROM RELOAD IS MISSING!!!
            ...
            pathTaken.setMap(psgeoMap) // XXXX THIS ADDS. IT SHOULD REPLACE!!!
