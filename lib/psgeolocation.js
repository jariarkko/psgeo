...
//     Closing that infowindow (if no line tracing takes place either) also stops the geolocation background worker
//     - no markers for position = no location service.
...
function iterateOverTracks(lib,lang,psgeoMap,fn,sharedCounter,sharedOutput,trackStartDate) {
    ...
    //
    //   2) If fn ever starts anything asynchronous, several processes may run in parallel.
...
function positionTrackingMenuInit(lib,lang,cfg,psgeoMap,psgeoDomainDefault,measureDistanceStart,trackStartDate) {
    ...
    // 5) Add a container for GPX download links
    ...
    // 6) Add a button to start distance measurement
    positionTrackingMenuDiv.appendChild(document.createElement('br'));
    let measureDistanceButton = document.createElement("BUTTON");
    measureDistanceButton.appendChild(document.createTextNode(lang.getText('measureDistanceButton')));
    ...
    positionTrackingMenuDiv.appendChild(measureDistanceButton)
    positionTrackingMenuDiv.appendChild(document.createElement('br'));
