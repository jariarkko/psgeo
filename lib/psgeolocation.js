...
function createGPX(trackName,trace) {
    ...
    alert("Making GPX. Trace length is "+trace.length)
    ...
    for (const pt of trace) {
        ...
        if (pt.alt !== null && pt.alt !== undefined) {
...
function PsgeoLocation() {
    ...
    function getGeolocationButton(lang,
        ...
        async function geolocationAchieved(position) {
            ...
            //     Include altitude (which might be undefined sometimes?)
            if (psgeoDomainDefault.locationTracking.trackRecording) await dbAppendTrackRecord(trackId,trackDate,LatLng,myPosition.alt)
