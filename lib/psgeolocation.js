...
//     iterateOverTracks(lib,trackStartDate,psgeoMap,fn) runs fn(lib,trackData,psgeoMap)
//     sequentially for each track.
...
function restoreTraceFromDb(lib,trackStartDate,psgeoMap) {
    ...
    const fn = function(lib,trackData,psgeoMap) {
        var trace = []
    ...
    iterateOverTracks(lib,trackStartDate,psgeoMap,fn)
}


function iterateOverTracks(lib,trackStartDate,psgeoMap,fn) {
    ...
    psgeoDebug("Loading recorded track data for "+trackStartDate)
        ...
        request.onsuccess = function () {
            ...
            psgeoDebug('Reading track index for date '+trackStartDate)
            var tracksRequest = trackListIndex.getAll([trackStartDate])
            tracksRequest.onfailure = function (e) {
                console.log(e) // ZZZZZZ
                db.close()
            }
            tracksRequest.onsuccess = function () {
                for (const track of tracksRequest.result) {
                    psgeoDebug('Reading single track data')
                    ...
                    trackDataRequest.onsuccess = function (e) {
                        ...
                        // -----------OPERATE--ON--DATA--------------------
                        if (trackData.length > 0) fn(lib,trackData,psgeoMap) // A track can be empty!
                        // ------------------------------------------------
                    }
                }
            }
...
function getFileList(lib,trackStartDate,psgeoMap) {

    let tracksForDownloadDiv = document.createElement('div')

    let fn = function (lib,trackData,psgeoMap) {
        // iterateOverTracks runs any function fn passed as indirection
        // with these parameters: lib,trackData,psgeoMap
        // trackData is an array of objects { timestamp, position, trackId }

        let trace2 = []
        let trackName = trackData[0].trackId
        for (const trackPoint of trackData) {
            let timestamp = trackPoint.timestamp
            trace2.push(trackPoint.position)
        }
        psgeoDebug('Iterating. '+trackName)
        const link = getTrackDownloadLink(trackName,trace2)
        const br = document.createElement('br')
        tracksForDownloadDiv.appendChild(link)
        tracksForDownloadDiv.appendChild(br)
    }

    iterateOverTracks(lib,trackStartDate,psgeoMap,fn)
    psgeoDebug('tracksForDownload div length is '+tracksForDownloadDiv.childNodes.length)
    return tracksForDownloadDiv
}
...
function PsgeoLocation() {
    ...
    function getGeolocationButton(lang,
        ...
        async function geolocationPending() {
            ...
            await restoreTraceFromDb(lib,trackStartDate,psgeoMap)
            ...
            let info = getFileList(lib,trackStartDate,psgeoMap) // TEMPORARY XXXXX ZZZZZ
        }
