...
function createGPX(trackName,trace) {
    ...
    for (const pt of trace) {
        // trace is an array of positions: [ { lat: "LAT", lng: "LNG" } , ... ] or [ { lat: "LAT", lng: "LNG alt: "ALT"} , ... ]
        // <trkpt lat="LATITUDE" lon="LONGITUDE"><ele>ELEVATION</ele><time>TIME</time></trkpt>
        const trkpt = '<trkpt lat="' + pt.lat + '" lon="' + pt.lng + '">'
        if (pt.alt !== null && ps.alt !== undefined) {
            trkpt += '<ele>' + pt.alt + '</ele>'
        }
...
function positionTrackingMenuInit(lib,lang,cfg,psgeoMap,psgeoDomainDefault,trackStartDate) {
    ...
    let idArray = [ "showPositionMarker","showTrack","showPositionMarkerAndTrack" ] // ZZZZZ
    ...
    let checkedIndex = 2
    if (psgeoDomainDefault.locationTracking.showPositionMarker && !psgeoDomainDefault.locationTracking.showTrack) checkedIndex = 0
    if (!psgeoDomainDefault.locationTracking.showPositionMarker && psgeoDomainDefault.locationTracking.showTrack) checkedIndex = 1
    ...
    // 4) Add a container for GPX download links
...
function PsgeoLocation() {
    ...
    function getGeolocationButton(lang,
        ...
        // HOWEVER
        //     objects might be visible and it's not necessary to pass them as parameters then!
        ...
        async function geolocationAchieved(position) {
            ...
            if (psgeoDomainDefault.locationTracking.trackRecording) await dbAppendTrackRecord(trackId,trackDate,LatLng)
