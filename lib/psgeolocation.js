...
function openConnectionToIndexedDb() {
    ...
    request.onupgradeneeded = function () {
        ...
        // Create a 'table' of recorded tracks
        //     date + name of objectstore (name of object store is YYYY-MM-DDTHH:MM:SSZ)
        var recordedTracks = db.createObjectStore("recordedTracks", { keyPath: "track" }) // Avoid TDZ
        recordedTracks.createIndex("date", ["date"], { unique: false })

        // Individual track recordings will be placed in their respective object stores,
        // which are created when needed = at the beginning of geolocation.
    }

    return request
}

function dbCreateTrack() {

    const posixTime = new Date()                 // seconds since 1.1.1970
    const isoTimestamp = posixTime.toISOString() // YYYY-MM-DDTHH:MM:SSZ start time identifies a track.
    ...
    request.onsuccess = function () {
        ...
        var objectStore = transaction.objectStore("recordedTracks")
        var putPromise = objectStore.put({ date: isoDate, track: isoTimestamp }) // track is id
        putPromise.onsuccess = () => dbClose()
        putPromise.onfailure = () => dbClose()
    }
    // return isoTimestamp // the name of the object store to use
}
