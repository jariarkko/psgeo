...
async function showTrackRecordingDialog(lib,lang,cfg,psgeoMap,psgeoDomainDefault,trackStartDate) { 
    ...
    // 1) Add a close button to the dialog
    ...
    let buttons = psgeoGetGenericRadioGroup(lang,radioGroup,idArray,fnArray,tracksForDownloadDiv,psgeoDomainDefault)
    ...
    // 4) Define a handler for the database records
    ...
    var fn = function(lib,lang,trackData,psgeoMap,sharedCounter,sharedOutput) {
        ...
        tracksToProcess.value--
            ...
            console.log(sharedOutput) // ZZZZZZ
    ...
    // 5) And run download link generation!
...
function PsgeoLocation() {
    ...
    function getGeolocationButton(lang,
        ...
        async function geolocationAchieved(position) {
            ...
            console.log(psgeoDomainDefault.locationTracking.showPositionMarker)

            const firstAcceptedPosition = 2
