...
// Note: FilterMenu comes with detailed filtering options. MoreMenu (i) comes with additional info.
...
function psgeoFilterMenuBringUp(db,lib,lang,map,toOpen,psgeoSmallDisplayMode,myPosition) {
    ...
    if (!psgeoDomainDefault.classicUI) {
        psgeoGenericMenuButton(lang,'',lang.getText("close"),div, function () { // XXXX
    ...
    if (toOpen) {
        ...
        if (psgeoDomainDefault.classicUI) {
            ...
            // Note: this window is closed by 
            //     if (!psgeoDomainDefault.classicUI) map.controls[google.maps.ControlPosition.BOTTOM_CENTER].clear();
            // when a close button is clicked or the map is clicked.
            let moreFilterWindow = document.getElementById('MoreFilter');
