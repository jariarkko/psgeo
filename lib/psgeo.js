...
var psgeoDomainDefaults = [
	...
	hideButtontextFilter: false,
	hideButtontextMore: false,
	imgDefaultSize: 30,
	imgFilter: "./icons/neu_interface_find_magnifying_glass_icon_149474.svg",	// Filtermenu
	imgMore: "./icons/neu_interface_info_icon_149458.svg",				// MoreMenu
	imgAbout: "./icons/neu_interface_info_icon_149458.svg",				// 	About
	imgCaveClassification: "./icons/neu_interface_faq_question_icon_149479.svg",	// 	Help/Reference = Classification of caves
	imgOtherTools: "./icons/direction_gps.png",					// 	External resources = Other maps
	imgCaveAdd: "./icons/neu_interface_add_circle_insert_plus_icon_149548.svg"	// 	Action link = AddCave
    },
    {
	...
	// Domain specific images imgXxxx for buttons
	imgDefaultSize: 30,								// 30px is good default size	
	// imgFilter: "./icons/neu_interface_filter_funnel_icon_149475.svg",		// Filtermenu
	imgFilter: "./icons/neu_interface_find_magnifying_glass_icon_149474.svg",	// Filtermenu
	imgMore: "./icons/neu_interface_info_icon_149458.svg",				// MoreMenu
	imgAbout: "./icons/neu_interface_info_icon_149458.svg",				// 	About
	imgCaveClassification: "./icons/neu_interface_faq_question_icon_149479.svg",	// 	Help/Reference = Classification of caves
...
function psgeoCreateCaveSizeCheckbox(db,lang,document,subsubdiv1b,size,name,br) {
    var box = psgeoGetGenericCheckbox(size,lang.capitalize(name),subsubdiv1b,br);
    psgeoFilterCheckboxes.push(box);
//    var label= document.createElement("label");
...
function psgeoInitFilterPaneText(db,lib,lang,map,div) {
    ...
    if (psgeoSmallDisplayMode && items.length > 0) {
        if (psgeoDomainDefault.cityselection) {
            psgeoFilterSectionPrefix(lang,lang.cityText(),subsubdiv1b);
        } else {
    ...
    if (psgeoDomainDefault.cavemaps) {
        ...
        if (psgeoDomainDefault.finlandselection) {
            if (psgeoSmallDisplayMode) psgeoFilterSectionPrefix(lang,lang.countryText(),subsubdiv1b);
...
function psgeoRerunFilters(db,lib,lang,map) {
    ...
    for (i = 0; i < psgeoFilterCheckboxes.length; i++) {
        ...
        var checked = psgeoFilterCheckboxes[i].checked;
...
function psgeoUpdateStatsPaneText(db,
    ...
    // On sites that display cavesurveys on the map (Luolaseura.fi and Planetcaver.net Map)
    // do not count "places" (e.g. caves) on a small display. IS THIS A GOOD CHOICE??? DONT WE WANT THAT NUMBER?
...
function psgeoInitStatsPane(db,lib,lang,map) {
    if (!psgeoSmallDisplayMode) {
        psgeoInitFilterPaneText(db,lib,lang,map,psgeoFilterPaneTexts[0]);
    }
    psgeoUpdateStatsPane(db,lib,lang,map);
}
...
function psgeoFilterMenuContents(db,lib,lang,map,subdiv) {
    ...
    if (psgeoSmallDisplayMode) {
        psgeoInitFilterPaneText(db,lib,lang,map,subdiv);
    }
...
function psgeoGenericMenuButton(imgurl,lang,text,structure,fn,spacer) {	// RALF
	...
	if ( imgurl === undefined || imgurl == "" ) {				// some buttons have no icon, only text
		...
		if ( text === undefined || text == "" ) {			//	and ONLY an icon
			label.innerHTML = '<img src="' + imgurl + '" width=' + psgeoDomainDefault.imgDefaultSize + '>';	// COULD BE IN CSS!!!
			}
		else {								// there is both an icon and a text for the button
			label.innerHTML = '<img src="' + imgurl + '" width=' + psgeoDomainDefault.imgDefaultSize + '> ' + text;
...
function psgeoGetOtherToolsMenuButton(lang,structure,fn) {		// RALF
	var icon = psgeoDomainDefault.imgOtherTools;
	if ( icon === undefined ) { icon=""; }
	return(psgeoGenericMenuButton(icon,lang,lang.moreMapsText(),structure,fn,"br"));
	}	
    
function psgeoGetFilterMenuButton(lang,structure,fn) {			// RALF
	var icon = psgeoDomainDefault.imgFilter; 
	if ( icon === undefined ) { icon=""; }
	if ( psgeoDomainDefault.hideButtontextFilter ) {
		return(psgeoGenericMenuButton(icon,lang,"",structure,fn,"br"));
	}
	else {
		return(psgeoGenericMenuButton(icon,lang,
...
function psgeoGetMoreMenuButton(lang,structure,fn) {			// RALF
	var icon = psgeoDomainDefault.imgMore;
	if ( icon === undefined ) { icon=""; }
	if ( psgeoDomainDefault.hideButtontextMore ) {
		return(psgeoGenericMenuButton(icon,lang,"",structure,fn,"br"));
	}
	else {
		return(psgeoGenericMenuButton(icon,lang,lang.toolsAndAboutText(),structure,fn,"br"));
	}
}
    
function psgeoGetAboutMenuButton(lang,structure,fn) {			// RALF
	var icon = psgeoDomainDefault.imgAbout;
	if ( icon === undefined ) { icon=""; }
	return(psgeoGenericMenuButton(icon,lang,lang.aboutText(),structure,fn,"br"));
}	

function psgeoGetCaveAddMenuButton(lang,structure,fn) {			// RALF
	var icon = psgeoDomainDefault.imgCaveAdd;
	if ( icon === undefined ) { icon=""; }
	return(psgeoGenericMenuButton(icon,lang,					
...
function psgeoGetCaveClassificationMenuButton(lang,structure,fn) {	// RALF
	var icon = psgeoDomainDefault.imgCaveClassification;
	if ( icon === undefined ) { icon=""; }
	return(psgeoGenericMenuButton(icon,lang,lang.caveClassificationText(),structure,fn,"br"));
}
...
function psgeoGetGenericCheckbox(id,text,structure,br) {
    var label = document.createElement("label");
...
function psgeoGetWaterbodyCheckboxes(lib,lang,structure) {
    ...
    for (var i = 0; i < types.length; i++) {
        ...
        box = psgeoGetGenericCheckbox(subact,name,structure,false);
        result.push(box);
    }
    box = psgeoGetGenericCheckbox("Balls",lang.ballsText(),structure,false);
    result.push(box);
    box = psgeoGetGenericCheckbox("Ice",lang.iceText(),structure,false);
    result.push(box);
    box = psgeoGetGenericCheckbox("Waterbody-Unknown",lang.swimmingPlaceValueText("waterbody-unknown"),structure,false);
    result.push(box);
    return(result);
}
...
function psgeoGetRockTypeCheckboxes(lib,lang,structure) {
    ...
    var result = [];
    var box;
    box = psgeoGetGenericCheckbox("Other-Material",lang.otherMaterialText(),structure,false);
    result.push(box);
    box = psgeoGetGenericCheckbox("Ice",lang.iceText(),structure,false);
    result.push(box);
    for (var i = 0; i < types.length; i++) {
        ...
        box = psgeoGetGenericCheckbox(subact,name,structure,(i == types.length - 1));
        result.push(box);
    }
    return(result);
}
...
function psgeoInitStatsPaneControl(map) {
    ...
    //var statsPaneDiv = document.getElementById('Info');
    //if (statsPaneDiv === undefined || statsPaneDiv === null)
    //statsPaneDiv = document.getElementById('Planetcaver');
    ...
    if (!psgeoSmallDisplayMode) {
        //var filterPaneDiv = document.getElementById('Filter');
