...
function psgeoUpdateStatsPaneText(db,
    ...
    if (psgeoGeolocationButton === undefined) {
        ...
        if (psgeoDomainDefault.geolocationContinuousUpdate) {
         ...
         // HERE WE COULD HAVE A POSITION CHANGED LISTENER THAT WOULD UPDATE myPosition // DEBUG XXXXX
         // google.maps.event.addListener(markerMyPosition, 'position_changed', function() {
         //    myPosition = markerMyPosition.getPosition();
         // });
        ...
        function geolocationAchieved(position) {
            ...
            // Center map when the position is first marked, but do not constantly recenter the map. That would be annoying.
            // Note, that autopanning was also set to false in markerMyPosition definition.
            if (markerMyPosition.getContent() === lang.getText("geolocationWaitForPosition")) {
               // First position fix. Center map here.
               psgeoMap.setCenter(myPosition);
               // And do NOT do any map centering on successive updates. That would make the map VERY annoying to use.
