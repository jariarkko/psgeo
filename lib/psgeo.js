...
function psgeoOtherToolsMenuBringUp(db,lib,lang,map) {
    ...
    if (!psgeoDomainDefault.classicUI || psgeoOtherToolsWindow === undefined) { // YYYYY
...
function psgeoFilterMenuContents(db,lib,lang,map,subdiv,myPosition) {
    ...
    subdiv.addEventListener('click', function() {    // XXXXX pointerdown
...
function psgeoGetSourcesCheckboxes(lib,lang,structure) {
    ...
    var result = [];
    var sourcesLeft = psgeoSources.length; // YYYYYYY
    for (const source of psgeoSources ) {
