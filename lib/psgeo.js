...
function psgeoRockTypeText(lang,prefix,list) {
    ...
    for (var i = 0; i < list.length; i++) {
        if (i > 0) text += "/";
        text += list[i].substring(prefix.length);
    }
    return(text);
}
...
function psgeoMarkerPopupAuxRLItem(db,lang,item,rli) {
    if (db.getReadingListItemURL(rli) !== undefined) {
        return(psgeoPopupTextAndLink(lang,
...
function psgeoMarkerPopup(map,marker,popup,db,lang) {
    ...
    var rocktype = db.getItemSubActivitiesFiltered(item,"Caving",rocktypeprefix);
    var morphology = db.getItemSubActivitiesFiltered(item,"Caving",morphologyprefix);
