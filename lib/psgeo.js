...
function psgeoMarkerClick(marker,myPosition,cfg,psgeoDomainDefault) {
    var db = $.psgeoDB;
    var lib = $.psgeoLib;
    var lang = $.psgeoLang;
...
function psgeoInitMarker(db,lib,lang,item,filter,myPosition,cfg,psgeoDomainDefault) {
    ...
    google.maps.event.addListener(marker, 'click',
                                  function() {
                                      ...
                                      psgeoMarkerClick(marker,myPosition,cfg,psgeoDomainDefault);
                                  });
...
function psgeoInitFilterPaneText(db,lib,lang,map,div,smallPane,myPosition,tbl,cfg,psgeoDomainDefault) {
    ...
    div.addEventListener('click', function() {
        ...
        psgeoRerunFilters(db,lib,lang,map,myPosition,tbl,cfg,psgeoDomainDefault);
    });
...
function psgeoRerunFilters(db,lib,lang,map,myPosition,tbl,cfg,psgeoDomainDefault) {
    ...
    psgeoUpdateStatsPane(db,lib,lang,map,myPosition,tbl,cfg,psgeoDomainDefault);
}
        ...
        psgeoFilterMenuButton =
            psgeoGetFilterMenuButton(lang,
                                     ...
                                     function() {
                                         ...
                                         psgeoFilterMenuBringUp(db,lib,lang,map,true,psgeoSmallDisplayMode,myPosition,tbl,cfg,psgeoDomainDefault);
                                     },
                                     cfg,
                                     psgeoDomainDefault);
    }
        ...
        psgeoMoreMenuButton =
            psgeoGetMoreMenuButton(lang,
                                   ...
                                   function() {
                                       ...
                                       psgeoMoreMenuBringUp(db,lib,lang,map,true,cfg,psgeoDomainDefault);
                                   },
                                   cfg,
                                   psgeoDomainDefault);
    } 
...
function psgeoUpdateStatsPane(db,lib,lang,map,myPosition,tbl,cfg,psgeoDomainDefault) {
...
function psgeoInitStatsPane(db,lib,lang,map,myPosition,tbl,cfg,psgeoDomainDefault) {
    if (!psgeoSmallDisplayMode) {
        // create the 'small pane' filter window (upper window: length selector)
        psgeoInitFilterPaneText(db,lib,lang,map,psgeoFilterPaneTexts[0],true,myPosition,tbl,cfg,psgeoDomainDefault);
    }
    psgeoUpdateStatsPane(db,lib,lang,map,myPosition,tbl,cfg,psgeoDomainDefault);
}

function psgeoInitList(db,lib,lang,map,myPosition,tbl,cfg,psgeoDomainDefault) {
    ...
    psgeoDomainDefault.sources.forEach(function (source) { 
        ...
        $.getJSON(source.datafile,function(data) {
            ...
            if (--jsonImportsToDo == 0) psgeoFinishLoading(db,lib,lang,map,filterForMarkers,myPosition,tbl,cfg,psgeoDomainDefault);
        });
    });
}
...
function psgeoFinishLoading(db,lib,lang,map,filterForMarkers,myPosition,tbl,cfg,psgeoDomainDefault) {
    ...
    db.applyAllNoFilter(function(item) {
        psgeoInitMarker(db,lib,lang,item,filterForMarkers,myPosition,cfg,psgeoDomainDefault);
    });
    psgeoInitStatsPane(db,lib,lang,map,myPosition,tbl,cfg,psgeoDomainDefault);
    psgeoDebug("Loaded");
    psgeoInitPlace(db,lib,lang,map,new URLSearchParams(window.location.search),myPosition,cfg);
}
...
function psgeoOtherToolsMenuBringUp(db,lib,lang,map,cfg,psgeoDomainDefault) {
...
function psgeoFilterMenuContents(db,lib,lang,map,subdiv,myPosition,tbl,cfg,psgeoDomainDefault) {
    ...
    if (psgeoSmallDisplayMode) { // On small displays we only have the range filter on this popup window
        psgeoInitFilterPaneText(db,lib,lang,map,subdiv,false,myPosition,tbl,cfg,psgeoDomainDefault); 
    }
...
function psgeoMoreMenuContents(db,lib,lang,map,subdiv,cfg,psgeoDomainDefault) {
            ...
            psgeoOtherToolsMenuButton =
                psgeoGetOtherToolsMenuButton(lang,
                                             ...
                                             function() {
                                                 ...
                                                 psgeoOtherToolsMenuBringUp(db,lib,lang,map,cfg,psgeoDomainDefault);
                                             },
                                             cfg,
                                             psgeoDomainDefault);
        }
        ...
        psgeoAboutMenuButton =
            psgeoGetAboutMenuButton(lang,
                                    ...
                                    function() {
                                        ...
                                        psgeoAboutMenuBringUp(db,lib,lang,map,cfg,psgeoDomainDefault);
                                    },
                                    cfg,                                    
                                    psgeoDomainDefault);
    }
...
function psgeoFilterMenuBringUp(db,lib,lang,map,toOpen,psgeoSmallDisplayMode,myPosition,tbl,cfg,psgeoDomainDefault) {
...
if (!psgeoDomainDefault.classicUI && psgeoFilterWindow === undefined) {
    ...
    div.appendChild(document.createElement("br"));
    psgeoFilterMenuContents(db,lib,lang,map,subdiv,myPosition,tbl,cfg,psgeoDomainDefault);
...
function psgeoMoreMenuBringUp(db,lib,lang,map,toOpen,cfg,psgeoDomainDefault) { // Lisätiedot ja työkalut (i)
    ...
    if (!psgeoDomainDefault.classicUI || psgeoMoreWindow === undefined) {
        ...
        psgeoMoreMenuContents(db,lib,lang,map,subdiv,cfg,psgeoDomainDefault);
...
function psgeoAboutMenuBringUp(db,lib,lang,map,cfg,psgeoDomainDefault) { 
...
function psgeoInitPlaceAux(db,lib,lang,map,param,style,changeCenter,changeZoom,myPosition,cfg) {
...
function psgeoInitPlace(db,lib,lang,map,urlParams,myPosition,cfg) {
    ...
    if (image != null) {
        psgeoInitPlaceAux(db,lib,lang,map,image,"image",true,true,myPosition,cfg);
    } else if (maprequest != null) {
        psgeoInitPlaceAux(db,lib,lang,map,maprequest,"map",true,true,myPosition,cfg);
    } else if (model != null) {
        psgeoInitPlaceAux(db,lib,lang,map,model,"model",true,true,myPositio,cfg);
    }  else if (popup !== null) {
        psgeoInitPlaceAux(db,lib,lang,map,popup,"popup",true,true,myPosition,cfg);
    } else if (place !== null) {
        psgeoInitPlaceAux(db,lib,lang,map,place,"center",true,true,myPosition,cfg);
    }
}
...
function psgeoInitMapFinnishTerrain(lang,mapLegend,baseUrl,apiKey,psgeoSmallDisplayMode) {
...
function psgeoInitMapSwedishTerrain(lang,mapLegend,baseUrl,apiKey,psgeoSmallDisplayMode) {
...
function psgeoInitLinkHandlers(db,lib,lang,map,myPosition,cfg) {
    ...
    $(document).on("onpoiterdown", "a", function() {
        ...
        if (begin === "?p=") {
            ...
            psgeoInitPlaceAux(db,lib,lang,map,rest,"popup",false,false,myPosition,cfg);
            return(false);
        } else if (begin === "?i=") {
            var rest = href.substring(3);
            psgeoInitPlaceAux(db,lib,lang,map,rest,"image",false,false,myPosition,cfg);
            return(false);
        } else if (begin === "?m=") {
            var rest = href.substring(3);
            psgeoInitPlaceAux(db,lib,lang,map,rest,"map",false,false,myPosition,cfg);
            return(false);
        } else if (begin === "?o=") {
            var rest = href.substring(3);
           psgeoInitPlaceAux(db,lib,lang,map,rest,"model",false,false,myPosition,cfg);
            return(false);
        } else {
            return(true);
        }
...
async function psgeoInit() {
    ...
    psgeoInitMap(lib,db,cfg,tbl,psgeoDomainDefault,queryString,urlParams,pathname,hostname);
}

function psgeoInitMap(lib,db,cfg,tbl,psgeoDomainDefault,queryString, urlParams,pathname,hostname) {

    //    Icons and domain dependent settings.
    const psgeoSources = psgeoDomainDefault.sources; // where the JSON is retrieved from

    //    Some settings need to be assigned dynamically
    psgeoSmallDisplayMode = psgeoInitDisplaySize(cfg); // true or false
    psgeoInitLanguage(urlParams,psgeoDomainDefault);   // query string lang overrides Domain Default
    psgeoInitCavemaps(pathname,urlParams,psgeoDomainDefault);
    psgeoForceDisplayOfCustomLinks(hostname,urlParams,psgeoDomainDefault); // if query string luolaseura
    ...
    //
    // Onboard the Finnish topographic map // XXXXXXXX We could loop over topomaps!
    //
    var finnishMapTiming = lib.timingInitialize("psgeoInitMapFinnishTerrain");
    if (psgeoDomainDefault.topographicMaps.Finland.show) {

        let baseUrl = psgeoDomainDefault.topographicMaps.Finland.baseUrl;
        let apiKey = psgeoDomainDefault.topographicMaps.Finland.apiKey;
        let mapLegend = psgeoDomainDefault.topographicMaps.Finland.textId;

        $.psgeoMapFinnish = psgeoInitMapFinnishTerrain(lang,mapLegend,baseUrl,apiKey,psgeoSmallDisplayMode);
        psgeoMap.mapTypes.set(lang.getText(mapLegend,psgeoSmallDisplayMode), $.psgeoMapFinnish);
        psgeoMap.mapTypeControlOptions.mapTypeIds[2] = lang.getText(mapLegend,psgeoSmallDisplayMode);
    }
    lib.timingReport(finnishMapTiming);

    //
    // Onboard the Swedish topographic map
    //
    var swedishMapTiming = lib.timingInitialize("psgeoInitMapSwedishTerrain");
    if (psgeoDomainDefault.topographicMaps.Sweden.show) { 

        let baseUrl = psgeoDomainDefault.topographicMaps.Sweden.baseUrl;
        let apiKey = psgeoDomainDefault.topographicMaps.Sweden.apiKey;
        let mapLegend = psgeoDomainDefault.topographicMaps.Sweden.textId;

        $.psgeoMapSwedish = psgeoInitMapSwedishTerrain(lang,mapLegend,baseUrl,apiKey,psgeoSmallDisplayMode);
        psgeoMap.mapTypes.set(lang.getText(mapLegend,psgeoSmallDisplayMode), $.psgeoMapSwedish);
        psgeoMap.mapTypeControlOptions.mapTypeIds[3] = lang.getText(mapLegend,psgeoSmallDisplayMode);
    }
    lib.timingReport(swedishMapTiming);
    ...
    //
    psgeoDebug("initList...");
    psgeoInitList(db, lib,lang,psgeoMap,myPosition,tbl,cfg,psgeoDomainDefault);

    //
    // Initialize link handlers
    //
    psgeoDebug("initLinkClicks");
    psgeoInitLinkHandlers($.psgeoDB,$.psgeoLib,lang,psgeoMap,myPosition,cfg);
