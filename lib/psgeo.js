...
function psgeoPopupTitle(lib,lang,text) {
    return(lib.htmlParagraph(lib.htmlBold(text)));
}

function psgeoPopupImage(lib,lang,url) {
    return(lib.htmlImage(url,'width="140"'));
}

function psgeoPopupText(lib,lang,text) {
    return(lib.htmlParagraph(text));
}

function psgeoPopupTextLine(lib,lang,text) {
    return(text + lib.htmlBreak());
}

function psgeoPopupTextAndLink(lib,lang,text,link,url) {
...
function psgeoPopupTextAndLinkAndText(lib,lang,text1,link,url,text2) {
...
function psgeoPopupCoordinates(lib,lang,lat,lon) {
    ...
    var text = psgeoPopupTextAndLink(lib,lang,textHeader,textCoords,urlCoords);
    return(lib.htmlParagraph(text));
}
...
function psgeoMarkerPopupAuxRLItemAuthors(db,lang,rli) {
    ...
    if (auth.length == 0) return("");
        ...
        return(psgeoMarkerPopupAuxRLItemAuthorsInitials(auth[0]) +
               ...
               psgeoMarkerPopupAuxRLItemAuthorsInitials(auth[1]) +
               "&" +
               psgeoMarkerPopupAuxRLItemAuthorsInitials(auth[2]));
    else
        return(psgeoMarkerPopupAuxRLItemAuthorsInitials(auth[0]) +
               "," +
               psgeoMarkerPopupAuxRLItemAuthorsInitials(auth[1]) +
...
function psgeoMarkerPopupTextActivities(db,lib,lang,item) {
    ...
    if (movementissimple && otheractivitiesissimple) {
        ...
        return(psgeoPopupText(lib,lang,descr));
    }
}

function psgeoMarkerPopupTextMainCave(db,lib,lang,item) {
    if (db.hasItemMainCave(item)) {
        return(psgeoPopupText(lib,
                              lang,
                              lang.capitalize(lang.mainCaveText() + ": ") + db.getItemMainCave(item)));
    } else {
        return("");
    }
}

function psgeoMarkerPopupTextSideCaves(db,lib,lang,item) {
    var htmlInside = "";
    ...
    for (var i = 0; i < sidecaves.length; i++) {
        ...
        if (i+1 < sidecaves.length) thisHtml = psgeoPopupTextLine(lib,lang,thisHtml);
        htmlInside += thisHtml;
    }
    if (htmlInside.length > 0) {
        return(psgeoPopupText(lib,lang,htmlInside));
    } else {
        return("");
    }
}

function psgeoMarkerPopupAlternativeNames(db,lib,lang,item) {
    var an = db.getItemAlternativeNames(item);
    if (an.length > 0) {
        return(". " +
...
function psgeoMarkerPopupAlternativeMaps(db,lib,lang,item) {
    var result = "";
    var am = db.getItemAlternativeMaps(item);
    for (var i = 0; i < am.length; i++) {
        var cavemap = am[i];
        result += lib.htmlBreak();
        result += psgeoPopupTextAndLink(lib,
                                        lang,
...
function psgeoMarkerPopupAlternativeCoordinates(db,lib,lang,item) {
    var result = "";
    ...
    result += " (";
    result += lang.capitalize(db.getItemSource());
    result += "). ";
    result += lang.alternativeCoordinatesText();
    result += ": ";

    for (var i = 0; i < ac.length; i++) {
        var coord = ac[i];
        if (i > 0) result += ", ";
        result += psgeoPopupCoordinates(lib,
                                        lang,
                                        coord.lat
                                        coord.lon);
        result += " (";
...
function psgeoMarkerPopupText(db,lib,lang,item) {
    var rl = db.getItemReadingList(item);
    var html = "";
    html +=  psgeoPopupTitle(lib,lang,db.getItemName(item));
    if (db.hasItemReadingListImageUrl(item)) {
        var imageUrl = psgeoFindImageUrl(db,item,rl);
        html +=  psgeoPopupImage(lib,lang,imageUrl);
    }
    var descr = db.getItemDescriptionLang(item,lang.getLanguage());
    if (descr != undefined) {
        html += psgeoPopupText(lib,lang,descr);
    }
    if (db.getItemFuzzy(item)) {
        html += psgeoPopupText(lib,lang,lang.capitalize(lang.locationSecretText()));
    } else {
        var coordstext = psgeoPopupCoordinates(lib,
                                               lang,
                                               db.getItemLat(item),
                                               db.getItemLon(item));
        coordstext = psgeoMarkerPopupAlternativeCoordinatesText(db,lib,lang,item);
        html += coordstext;
    }
    var cavemap = db.getItemMap(item);
    if (cavemap !== undefined) {
        var maptext = "";
        maptext += psgeoPopupTextAndLink(lib,
                                         lang,
                                         lang.capitalize(lang.caveMapText() + ": "),
                                         lang.mapText(),
                                         cavemap);
        maptext += psgeoMarkerPopupAlternativeMaps(db,lib,lang,item);
        html += maptext;
    }
    ...
    if (db.getItemLength(item) !== undefined) {
        ...
        lengthtext += psgeoPopupAlternativeNames(db,lib,lang,item);
        html += psgeoPopupText(lib,
                               lang,
        ...
        typetext += psgeoPopupAlternativeNames(db,lib,lang,item);
        html += psgeoPopupText(lib,
                               lang,
                               typetext);
    }
    html += psgeoMarkerPopupTextActivities(db,lib,lang,item);
    var htmlinside = "";
    for (var i = 0; i < rl.length; i++) {
        var thishtml = psgeoMarkerPopupAuxRLItem(db,lib,lang,item,rl[i]);
        if (i+1 < rl.length) thishtml = psgeoPopupTextLine(lib,lang,thishtml);
        htmlinside += thishtml;
    }
    html += psgeoPopupText(lib,lang,htmlinside);
    html += psgeoMarkerPopupTextMainCave(db,lib,lang,item);
    html += psgeoMarkerPopupTextSideCaves(db,lib,lang,item);
    return(html);
}

function psgeoMarkerPopup(map,marker,popup,db,lib,lang) {
    var item = marker.item;
    var html = psgeoMarkerPopupText(db,lib,lang,item);
...
function psgeoAboutMenuBringUp(db,lib,lang,map) {
    ...
    if (psgeoAboutWindow === undefined) {
        ...
        var texts = [
            ...
            lang.developersNotResponsibleText()
        ];
