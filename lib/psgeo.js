...
// Swimming). The fields are as follows:
//
...
//
//   extraSubActivities: often there's the usual sub-activities,
//   followed by some special cases. The special cases can be listed
//   here, as an array of strings.
//
...
const psgeoSubtypeGroups = [
    {
        ...
        extraSubActivities: ["Waterbody-Unknown"],
    ...
    },
    {
        ...
        getSubActivitiesFunctionArgument: false,
        extraSubActivities: ["Unknown-substance"],
...
const psgeoIconFilter = "./icons/neu_interface_find_magnifying_glass_icon_149474.svg";	 // Filtermenu
const psgeoIconMore = "./icons/neu_interface_info_icon_149458.svg";                      // MoreMenu
const psgeoIconAbout = "./icons/neu_interface_info_icon_149458.svg";			 // About
const psgeoIconCaveClassification = "./icons/neu_interface_faq_question_icon_149479.svg";// Help/Reference = Classification of caves
const psgeoIconOtherTools = "./icons/direction_gps.png";                                 // External resources = Other maps
const psgeoIconCaveAdd = "./icons/neu_interface_add_circle_insert_plus_icon_149548.svg"; // Action link = AddCave
...
//var psgeoSaunaTypeFilterCheckboxes = [];
//var psgeoWaterbodyFilterCheckboxes = [];
var psgeoRockTypeFilterCheckboxes = [];
var psgeoMorphologyFilterCheckboxes = [];
...
function psgeoInitFilterPaneText(db,lib,lang,map,div,smallPane) {
    ...
    if (psgeoSmallDisplayMode && items.length > 0) {
        psgeoFilterSectionPrefix(lang,lang.activityText(),subsubdiv1b);
    }
    ...
    if (psgeoDomainDefault.cavemaps) {
        if (psgeoSmallDisplayMode) psgeoFilterSectionPrefix(lang,lang.sizeText(),subsubdiv1b);
        ...
        psgeoCreateCaveSizeCheckbox(db,
                                    ...
                                    subsubdiv1b,
                                    "caves",
                                    lang.cavesText(psgeoCaveSizeSmall,psgeoCaveSizeBig,psgeoSmallDisplayMode),
                                    !psgeoSmallDisplayMode)
        psgeoCreateCaveSizeCheckbox(db,
                                    lang,
                                    document,
                                    ...
                                    true)
        if (psgeoSmallDisplayMode) psgeoFilterSectionPrefix(lang,lang.mapText(),subsubdiv1b);
...
//function psgeoCreateWaterbodyFilter(db,lib,lang,map) {
//    if (psgeoWaterbodyFilterCheckboxes.length == 0) {
//        return(undefined);
//    } else if (psgeoAllBoxesChecked(psgeoWaterbodyFilterCheckboxes)) {
//        return(undefined);
//    } else {
//        return(psgeoCreateSubactivityFilterFromCheckboxes(db,"Swimming",psgeoWaterbodyFilterCheckboxes));
//    }
//}
...
//function psgeoCreateSaunaTypeFilter(db,lib,lang,map) {
//    if (psgeoSaunaTypeFilterCheckboxes.length == 0) {
//        return(undefined);
//    } else if (psgeoAllBoxesChecked(psgeoSaunaTypeFilterCheckboxes)) {
//        return(undefined);
//    } else {
//        return(psgeoCreateSubactivityFilterFromCheckboxes(db,"Sauna",psgeoSaunaTypeFilterCheckboxes));
//    }
//}
...
function psgeoCreateRockFilter(db,lib,lang,map) {
    if (psgeoRockTypeFilterCheckboxes.length == 0) {
        return(undefined);
    } else if (psgeoAllBoxesChecked(psgeoRockTypeFilterCheckboxes)) {
        return(undefined);
    } else {
        return(psgeoCreateSubactivityFilterFromCheckboxes(db,"Caving",psgeoRockTypeFilterCheckboxes));
    }
}

function psgeoCreateMorphologyFilter(db,lib,lang,map) {
    if (psgeoMorphologyFilterCheckboxes.length == 0) {
        return(undefined);
    } else if (psgeoAllBoxesChecked(psgeoMorphologyFilterCheckboxes)) {
        return(undefined);
    } else {
        return(psgeoCreateSubactivityFilterFromCheckboxes(db,"Caving",psgeoMorphologyFilterCheckboxes));
    }
}
...
function psgeoRerunFilters(db,lib,lang,map) {
    ...
    //var newWaterbodyFilter = psgeoCreateWaterbodyFilter(db,lib,lang,map);
    var newSubtypeGroupFilters = psgeoCreateSubtypeGroupFilters(db,lib,lang,map);
    //var newSaunaTypeFilter = psgeoCreateSaunaTypeFilter(db,lib,lang,map);
    var newRockFilter = psgeoCreateRockFilter(db,lib,lang,map);
    var newMorphologyFilter = psgeoCreateMorphologyFilter(db,lib,lang,map);
    ...
    //if (newWaterbodyFilter !== undefined) {
    //    finalFilter = db.filterAnd([finalFilter,newWaterbodyFilter]);
    //    psgeoDebug("after waterbody filter = " + db.filterToString(finalFilter));
    //}
    ...
    //    if (newSaunaTypeFilter !== undefined) {
    //        finalFilter = db.filterAnd([finalFilter,newSaunaTypeFilter]);
    //        psgeoDebug("after saunaType filter = " + db.filterToString(finalFilter));
    //    }
    if (newRockFilter !== undefined) {
        finalFilter = db.filterAnd([finalFilter,newRockFilter]);
        psgeoDebug("after rock filter = " + db.filterToString(finalFilter));
    }
    if (newMorphologyFilter !== undefined) {
        finalFilter = db.filterAnd([finalFilter,newMorphologyFilter]);
        psgeoDebug("after morphology filter = " + db.filterToString(finalFilter));
    }
...
function psgeoStatLineNumberOfCaves(lang,n) {
    return(lang.numberAlign(n,psgeoStatLineNumberAlign) + " " + lang.cavesText());
}

function psgeoStatLineNumberOfPlaces(lang,n) {
    return(lang.numberAlign(n,psgeoStatLineNumberAlign) + " " + lang.placesText());
}

function psgeoStatLineNumberOfCities(lang,n) {
    return(lang.numberAlign(n,psgeoStatLineNumberAlign) + " " + lang.citiesText());
}

function psgeoStatLineNumberOfCountries(lang,n) {
    return(lang.numberAlign(n,psgeoStatLineNumberAlign) + " " + lang.countriesText());
}

function psgeoStatLineNumberOfContinents(lang,n) {
    return(lang.numberAlign(n,psgeoStatLineNumberAlign) + " " + lang.continentsText());
}

function psgeoStatLineNumberOfStates(lang,n) {
    return(lang.numberAlign(n,psgeoStatLineNumberAlign) + " " + lang.statesText());
}

function psgeoStatLineNumberOfProvinces(lang,n) {
    return(lang.numberAlign(n,psgeoStatLineNumberAlign) + " " + lang.provincesText());
}

function psgeoStatLineNumberOfCaveMaps(lang,n) {
    return(lang.numberAlign(n,psgeoStatLineNumberAlign) + " " + lang.caveMapsText());
}
...
function psgeoUpdateStatsPaneText(db,
    ...
    //if (psgeoDomainDefault.cavemaps) {
    //arrayitems[0].innerHTML = (lang.numberAlign(nplaces,3) + " " + lang.cavesText());
    //} else {
    //arrayitems[0].innerHTML = (lang.numberAlign(nplaces,3) + " " + lang.placesText());
    //}

    // On large displays (only) cities and countries will be shown.
    //if (!psgeoSmallDisplayMode) arrayitems[1].innerHTML = (lang.numberAlign(ncities,3) + " " + lang.citiesText());
    //if (!psgeoSmallDisplayMode) arrayitems[2].innerHTML = (lang.numberAlign(ncountries,3) + " " + lang.countriesText());

    // On other sites than Luolaseuora or Planetcaver Mapmap, continents, states, provices... will be counted
    //if (!psgeoDomainDefault.cavemaps) {
    ...
    //}
    //
    //// Show the number of cave maps, where applicable
    //if (psgeoDomainDefault.cavemaps) {
...
var psgeoOtherTools = [
    {
        ...
        icon: "./icons/retkipaikka_vaaka.png",
    ...
    },
    {
        ...
        icon: "./icons/mml_suomi_logo_rgb.svg",
...
function psgeoFilterMenuContents(db,lib,lang,map,subdiv) {
    ...
    if (psgeoDomainDefault.nameSearch) {
        psgeoFilterSectionPrefix(lang,lang.nameText(),subdiv);
    ...
    // Waterbody searches (handled by the new generic code now)
    //if (psgeoDomainDefault.waterbodySearch) {
    //psgeoFilterSectionPrefix(lang,lang.waterbodyText(),subdiv)
    //psgeoWaterbodyFilterCheckboxes = psgeoGetWaterbodyCheckboxes(lib,lang,subdiv);
    //}
    ...
    for (var i = 0; i < psgeoSubtypeGroups.length; i++) {
        ...
        psgeoDebug("considering subtype group " + group.selector);
    ...
    //// Sauna type searches (handled by the new generic code now)
    //if (psgeoDomainDefault.saunaTypeSearch) {
    //psgeoFilterSectionPrefix(lang,lang.saunaTypeText(),subdiv)
    //psgeoSaunaTypeFilterCheckboxes = psgeoGetSaunaTypeCheckboxes(lib,lang,subdiv);
    //}
    
    // Rock type selection/filtering is not really needed on a small display!
    if (psgeoDomainDefault.rockTypeSearch && !psgeoSmallDisplayMode) {
        psgeoFilterSectionPrefix(lang,lang.rockTypeText(),subdiv);
        psgeoRockTypeFilterCheckboxes = psgeoGetRockTypeCheckboxes(lib,lang,subdiv);
    }
    
    // Type filtering, however, is important
    if (psgeoDomainDefault.morphologySearch) {
        psgeoFilterSectionPrefix(lang,lang.morphologyText(),subdiv);
        psgeoMorphologyFilterCheckboxes = psgeoGetMorphologyCheckboxes(lib,lang,subdiv);
    }
    
    // Country filtering
    if (psgeoDomainDefault.finlandselection && !psgeoSmallDisplayMode) {
        psgeoFilterSectionPrefix(lang,lang.countryText(),subdiv);
    ...
    if (psgeoDomainDefault.sourcesCheckboxes) {
        if (psgeoSourceFilterCheckboxes.length == 0) {
            if (psgeoDomainDefault.luolaseura) {
                psgeoFilterSectionPrefix(lang,lang.sourceText(),subdiv);
...
//                "</p>" +
//                postfix;
//            psgeoDebug("trailer/ls = " + subdiv.innerHTML);
//        } else {
//            subdiv.innerHTML =
//                prefix +
//                "<p>" +
//                lang.capitalize(lang.sourcesText()) + ": " +
//                psgeoGetSources() + "</p>" +
//                postfix;
//            psgeoDebug("trailer/fi = " + subdiv.innerHTML);
//        }
//    }
}
...
function psgeoFilterMenuBringUp(db,lib,lang,map,toOpen) {
    if (psgeoFilterWindow === undefined) {
        ...
        var div = document.createElement("div");
        var titleelement = document.createElement("h3");
        var titletext = document.createTextNode(lang.capitalize(lang.filterTextHeader()));
        titleelement.appendChild(titletext);
        div.appendChild(titleelement);
...
function psgeoMoreMenuBringUp(db,lib,lang,map,toOpen) {
    if (psgeoMoreWindow === undefined) {
        ...
        var div = document.createElement("div");
        var titleelement = document.createElement("h3");
        var titletext = document.createTextNode(lang.capitalize(lang.moreInformationText()));
        titleelement.appendChild(titletext);
        div.appendChild(titleelement);
        subdiv = document.createElement("div");
        subdiv.innerHTML = "";
...
function psgeoAboutMenuBringUp(db,lib,lang,map) {
    if (psgeoAboutWindow === undefined) {
        ...
        var dataExplanation = "";
        if (psgeoDomainDefault.luolaseura) {
            dataExplanation += lang.dataExplanationCavingAssociationText();
        } else {
            dataExplanation += lang.dataExplanationText();
            dataExplanation += " ";
...
function psgeoGenericMenuButton(lang,imgurl,text,structure,fn,spacer) {	// RALF
    var br = document.createElement("br");					// column of links, not row -> newline required
    var label = document.createElement("button");				// and the button is created
    var text = lang.capitalize(text);		
    // GENERATE TEXT ONLY / ICON ONLY / ICON+TEXT BUTTONS
    if ( imgurl === undefined || imgurl == "" ) {				// some buttons have no icon, only text
	if ( text == "" ) { text = "link text missing"; }		// if the text is empty too, we have a problem LOG? QUIT?
	label.innerHTML = text;
    } else {									// there is an icon for the button
	if ( text === undefined || text == "" ) {			//	and ONLY an icon
	    label.innerHTML = '<img src="' + imgurl + '" width=' + psgeoDomainDefault.icons.imgDefaultSize + '>';	// COULD BE IN CSS!!!
	} else {								// there is both an icon and a text for the button
	    label.innerHTML = '<img src="' + imgurl + '" width=' + psgeoDomainDefault.icons.imgDefaultSize + '> ' + text;
	    //label.style.vertical-align = "center";		// text needs to align with the (center of the) icon
	}
    }
    ...
    label.style.fontSize = "90%";						// really? smaller?
    if ( spacer == "br" ) { structure.appendChild(br); }			// a switch is possible here is more styles are needed
...
function psgeoGetOtherToolsMenuButton(lang,structure,fn) {
    ...
    return(psgeoGenericMenuButton(lang,icon,lang.moreMapsText(),structure,fn,"br"));
}	
...
function psgeoGetFilterMenuButton(lang,structure,fn) {
	...
	return(psgeoGenericMenuButton(lang,
                                      icon,
                                      (psgeoSmallDisplayMode ? lang.filterTextLink() : lang.moreFiltersText()),
                                      structure,
                                      fn,"br"));
    }
}
...
function psgeoGetMoreMenuButton(lang,structure,fn) {
	...
	return(psgeoGenericMenuButton(lang,icon,lang.toolsAndAboutText(),structure,fn,"br"));
    }
}
...
function psgeoGetAboutMenuButton(lang,structure,fn) {
    ...
    if ( icon === undefined ) { icon=""; }
    return(psgeoGenericMenuButton(lang,icon,lang.aboutText(),structure,fn,"br"));
}	
...
function psgeoGetCaveAddMenuButton(lang,structure,fn) {
    ...
    return(psgeoGenericMenuButton(lang,
                                  icon,
                                  lang.addText() +  " (" + lang.orModifyText() + ") "  + lang.caveText(),
                                  structure,
                                  fn,"br"));	
}
...
function psgeoGetCaveClassificationMenuButton(lang,structure,fn) {
    ...
    if ( icon === undefined ) { icon=""; }
    return(psgeoGenericMenuButton(lang,icon,lang.caveClassificationText(),structure,fn,"br"));
}
...
//function psgeoGetWaterbodyCheckboxes(lib,lang,structure) {
//    var types = lib.getSwimmingWaterbodySubActivities(false);
//    var prefix = "Waterbody-";
//    var result = [];
//    var box;
//    for (var i = 0; i < types.length; i++) {
//        var subact = types[i];
//        var name = lang.swimmingPlaceValueText(subact.toLowerCase());
//        box = psgeoGetGenericCheckbox(subact,name,structure,true,false);
//        result.push(box);
//    }
//    box = psgeoGetGenericCheckbox("Balls",lang.ballsText(),structure,true,false);
//    result.push(box);
//    box = psgeoGetGenericCheckbox("Ice",lang.iceText(),structure,true,false);
//    result.push(box);
//    box = psgeoGetGenericCheckbox("Waterbody-Unknown",lang.swimmingPlaceValueText("waterbody-unknown"),structure,true,true);
//    result.push(box);
//    return(result);
//}

function psgeoGetSubtypeGroupCheckboxList(lib,lang,group,structure) {
    var types = lib[group.getSubActivitiesFunction](group.getSubActivitiesFuntionArgument);
    var result = [];
    var box;
    for (var i = 0; i < types.length; i++) {
        var subact = types[i];
        var name = lang[group.subActivityTextFunction](subact.toLowerCase());
        box = psgeoGetGenericCheckbox(subact,name,structure,true,(i == types.length-1 && group.extraSubActivities.length == 0));
        result.push(box);
    }
    for (var j = 0; j < group.extraSubActivities.length; j++) {
        var subact = group.extraSubActivities[j];
        var name = lang[group.subActivityTextFunction](subact.toLowerCase());
        box = psgeoGetGenericCheckbox(subact,name,structure,true,(j == group.extraSubActivities.length-1));
        result.push(box);
    }
...
//function psgeoGetSaunaTypeCheckboxes(lib,lang,structure) {
//    var types = lib.getSaunaTypeSubActivities(false);
//    var result = [];
//    var box;
//    for (var i = 0; i < types.length; i++) {
//        var subact = types[i];
//        var name = lang.saunaTypeValueText(subact.toLowerCase());
//        box = psgeoGetGenericCheckbox(subact,name,structure,true,false);
//        result.push(box);
//    }
//    box = psgeoGetGenericCheckbox("Unknown",lang.saunaTypeValueText("unknown"),structure,true,true);
//    result.push(box);
//    return(result);
//}

function psgeoGetOnlyFinlandCheckbox(lib,lang,structure) {
    return(psgeoGetGenericCheckbox("only finland",lang.onlyFinlandText(),structure,false,true));
}

function psgeoGetRockTypeCheckboxes(lib,lang,structure) {
    var types = lib.getCavingRockTypeSubActivities(true);
    var prefix = "Rock-";
    var result = [];
    var box;
    box = psgeoGetGenericCheckbox("Other-Material",lang.otherMaterialText(),structure,true,false);
    result.push(box);
    box = psgeoGetGenericCheckbox("Ice",lang.iceText(),structure,true,false);
    result.push(box);
    for (var i = 0; i < types.length; i++) {
        var subact = types[i];
        var name = lang.rockTypeValueText(subact.substring(prefix.length).toLowerCase());
        box = psgeoGetGenericCheckbox(subact,name,structure,true,(i == types.length - 1));
        result.push(box);
    }
    return(result);
}

function psgeoGetMorphologyCheckboxes(lib,lang,structure) {
    var types = lib.getCavingMorphologySubActivities(true);
    var prefix = "Morphology-";
    var result = [];
    for (var i = 0; i < types.length; i++) {
        var subact = types[i];
        var name = lang.morphologyValueText(subact.substring(prefix.length).toLowerCase());
        box = psgeoGetGenericCheckbox(subact,name,structure,true,(i == types.length -1));
        result.push(box);
    }
    return(result);
