...
function psgeoUpdateStatsPaneText(db,
    ...
    if (psgeoGeolocationButton === undefined &&
        ...
        var geolocationButtonLocation = 
            psgeoSmallDisplayMode ?
            arrayitems[8] :            // Button to lower pane. Position: after 8th div.
            psgeoFilterPaneTexts[0];   // Button to upper pane. Position: after 1st div (with many labels).

        var geolocatorId;              // Worker id needed to stop it.
        var geolocationClicks = 0;     // Geolocation button is on/off toggle depending on %2
        const geolocationOptions = {maximumAge:10000, timeout:10000, enableHighAccuracy: true};

        if (psgeoDomainDefault.continuousGeolocation) {
            markerMyPosition = new google.maps.InfoWindow();
            google.maps.event.addListener(markerMyPosition, 'closeclick', function() {
               geolocationClicks++;                            // Keep the on/off toggle updated.
               navigator.geolocation.clearWatch(geolocatorId); // User closes infoWindow so stop updating it
               psgeoDebug("User closed markerMyPosition. Disabling continuous geolocation. Worker stopped. geolocatorId is now " + typeof(geolocatorId) + " and there have been " + geolocationClicks + " clicks.");
            });
        } else {
            markerMyPosition = new google.maps.InfoWindow();
        }
        ...
        function geolocationAchieved(position) {
            ...
            markerMyPosition.setContent(lang.getText("myGeolocation"));
            markerMyPosition.open(psgeoMap);
            psgeoMap.setCenter(myPosition);
	}

	function geolocationFailed() {
            markerMyPosition.setPosition(map.getCenter("Geolocation failed!"));
        }

        // Create a geolocation button and add onclick geolocation functionality
        psgeoGeolocationButton =
            psgeoGetGeolocationButton(
                lang,                        // for translation of text
                geolocationButtonLocation,   // upper (filter) or lower (info) pane
                function() {
                    geolocationClicks++;
                    psgeoDebug("Geolocation button clicked.");
                    if (navigator.geolocation && !psgeoDomainDefault.continuousGeolocation ) {
                    ...
                    } else if (navigator.geolocation && psgeoDomainDefault.continuousGeolocation ) {
                        if ( geolocationClicks %2 === 0 ) {                     // a geolocation worker exists. Embarrassing solution.
                            navigator.geolocation.clearWatch(geolocatorId);   // so this click was meant to stop it
                            markerMyPosition.setMap(null);                    // remove marker
                            psgeoDebug("Continuous geolocation disabled. Worker stopped. Marker removed. geolocatorId is now " + typeof(geolocatorId) + " and there have been " + geolocationClicks + " clicks.");
                        } else {
                            geolocatorId = navigator.geolocation.watchPosition(geolocationAchieved,geolocationFailed,geolocationOptions);
                            psgeoDebug("Continuous geolocation enabled. Worker id is " + geolocatorId + " and there have been " + geolocationClicks + " clicks.");
                        }
                    } else {
                        markerMyPosition.setPosition(map.getCenter("Geolocation not supported!"));
                    }
                 } // end function()
