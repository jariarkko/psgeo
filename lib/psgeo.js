...
const psgeoSourcesLuolaseura = [
    {
        textId: "Luolaseura",
        // link: "https://kartta.luolaseura.fi",
        // datafile: "https://kartta.luolaseura.fi/db/caves.luolaseura.json"
        link: "https://luolaseura.fi/luolakanta/kartta.html",
...
function psgeoMarkerPopup(map,marker,popup,db,lib,lang,myPosition) {
    psgeoMarkerPopupItem(map,marker.item,popup,db,lib,lang,"popup",myPosition);
}

function psgeoMarkerPopupItem(map,item,popup,db,lib,lang,style,myPosition) {
    var html = "";
    switch (style) {
    case "popup":
    ...
    case "map":
        ...
        if (mapUrl === undefined) {
            psgeoMarkerPopupItem(map,item,popup,db,lib,lang,"popup",myPosition);
        } else {
...
function psgeoMarkerClick(marker,myPosition) {
    var db = $.psgeoDB;
    var lib = $.psgeoLib;
    var lang = $.psgeoLang;
    psgeoMarkerPopup(psgeoMap,marker,$.psgeoPopup,db,lib,lang,myPosition);
}
...
function psgeoInitMarker(db,lib,lang,item,filter,myPosition) {
    ...
    google.maps.event.addListener(marker, 'click',
                                  function() {
                                      ...
                                      psgeoMarkerClick(marker,myPosition);
                                  });
...
function psgeoInitFilterPaneText(db,lib,lang,map,div,smallPane,myPosition) {
    ...
    if (psgeoDomainDefault.cavemaps) {
        ...
        psgeoCreateRangeCheckbox(db,
                                    ...
                                    lang.getTextWithValues("rangeMedium",
                                                           psgeoSmallDisplayMode,
                                                           [psgeoRange.limitLow,psgeoRange.limitHigh]),
                                    !psgeoSmallDisplayMode)
        psgeoCreateRangeCheckbox(db,
                                    lang,
                                    document,
    ...
    div.addEventListener('click', function() {
        ...
        psgeoRerunFilters(db,lib,lang,map,myPosition);
    });
...
function psgeoRerunFilters(db,lib,lang,map,myPosition) {
    var newBasicFilters = [];
    var newSizeFilters = [];
    ...
    for (i = 0; i < psgeoRangeFilterCheckboxes.length; i++) {
        var theCheckbox = psgeoRangeFilterCheckboxes[i];
        var checkValue = theCheckbox.value;
        var checked = theCheckbox.checked;
        if (checked) {
            var newFilter;
            switch (checkValue) {
            case "big caves":
                newFilter = db.filterRange("min",psgeoRange.limitHigh);
                newSizeFilters.push(newFilter);
                bigCaveFlag = true;
                break;
            case "caves":
                var newFilter1 = db.filterRange("min",psgeoRange.limitLow);
                var newFilter2 = db.filterRange("max",psgeoRange.limitHigh);
                newFilter = db.filterAnd([newFilter1,newFilter2]);
                newSizeFilters.push(newFilter);
                caveFlag = true;
                break;
            case "cavelets":
                newFilter = db.filterRange("max",psgeoRange.limitLow);
                newSizeFilters.push(newFilter);
                caveletFlag = true;
                break;
            default:
    ...
    psgeoUpdateStatsPane(db,lib,lang,map,myPosition);
}
        ...
        psgeoFilterMenuButton =
            psgeoGetFilterMenuButton(lang,
                                     ...
                                     function() {
                                         ...
                                         psgeoFilterMenuBringUp(db,lib,lang,map,true,psgeoSmallDisplayMode,myPosition);
                                     });
    }
...
function psgeoUpdateStatsPane(db,lib,lang,map,myPosition) {
...
function psgeoInitStatsPane(db,lib,lang,map,myPosition) {
    if (!psgeoSmallDisplayMode) {
...
function psgeoInitList(lib,lang,map,myPosition) {
    psgeoList = [];
    psgeoMarkerList = [];
    psgeoMarkerItemList = [];
    var db = PsgeoDB(lib,[],"");
    ...
    psgeoInitListOne(db,lib,lang,map,filterForMarkers,0,myPosition);
}

function psgeoInitListOne(db,lib,lang,map,filterForMarkers,count,myPosition) {
    if (count == psgeoSources.length) {
        psgeoDebug("added places");
        psgeoFinishLoading(db,lib,lang,map,filterForMarkers,myPosition);
    } else {
        ...
        $.getJSON(path, function(data) {
            ...
            psgeoInitListOne(db,lib,lang,map,filterForMarkers,count+1,myPosition);
        });
    }
}

function psgeoFinishLoading(db,lib,lang,map,filterForMarkers,myPosition) {
    ...
    db.applyAllNoFilter(function(item) {
        psgeoInitMarker(db,lib,lang,item,filterForMarkers,myPosition);
    });
    psgeoInitStatsPane(db,lib,lang,map,myPosition);
    psgeoDebug("Loaded");
...
function psgeoOtherToolsMenuBringUp(db,lib,lang,map) {
    ...
    if (!psgeoDomainDefault.classicUI || psgeoOtherToolsWindow === undefined) {
        ...
        const widthSmall = 'width="60"'        // XXXXX style
        ...
        var titletext = document.createTextNode(lang.capitalize(lang.getText("moreMaps"))); // XXXX moreMaps not a good term anymore
...
function psgeoFilterMenuContents(db,lib,lang,map,subdiv,myPosition) {
    ...
    subdiv.addEventListener('pointerdown', function() { 
...
function psgeoFilterMenuBringUp(db,lib,lang,map,toOpen,psgeoSmallDisplayMode,myPosition) {
...
if (!psgeoDomainDefault.classicUI && psgeoFilterWindow === undefined) {
    ...
    div.appendChild(document.createElement("br"));
    psgeoFilterMenuContents(db,lib,lang,map,subdiv,myPosition);
...
function psgeoMoreMenuBringUp(db,lib,lang,map,toOpen) { // Lisätiedot ja työkalut (i)
...
function psgeoAboutMenuBringUp(db,lib,lang,map) { // XXXXXXXXXXXX
    if (!psgeoDomainDefault.classicUI || psgeoAboutWindow === undefined) {
        psgeoAboutWindow = new google.maps.InfoWindow;
        var div = document.createElement("div");
        if (!psgeoDomainDefault.classicUI) {
            div.setAttribute("id", "AboutMenu"); // referenced to in css/style.css
            // Create a close button to the upper righthand corner
            psgeoGenericMenuButton(lang,'',lang.getText("close"),div, function () {
                map.controls[google.maps.ControlPosition.BOTTOM_CENTER].clear();
                },'','closeButton');
...
function psgeoGenericMenuButton(lang,imgurl,text,structure,fn,spacer,buttonId) {
    ...
    psgeoDebug("DEBUG Generic button: " + imgurl); // XXX
...
function psgeoInitPlaceAux(db,lib,lang,map,param,style,changeCenter,changeZoom) {
    ...
    if (items.length > 0) {
        ...
        case "center":
            break;
        case "popup":
            psgeoMarkerPopupItem(map,item,$.psgeoPopup,db,lib,lang,"popup",myPosition);
            break;
        case "image":
            psgeoMarkerPopupItem(map,item,$.psgeoPopup,db,lib,lang,"image",myPosition);
            break;
        case "map":
            psgeoMarkerPopupItem(map,item,$.psgeoPopup,db,lib,lang,"map",myPosition);
            break;
        case "model":
            psgeoMarkerPopupItem(map,item,$.psgeoPopup,db,lib,lang,"model",myPosition);
            break;
        default:
            psgeoDebug("ERROR: invalid style " + style);
        }
...
function psgeoInitLuolaseura(hostname,urlParams) {
    ...
    psgeoDebug("Fetched psgeoSources: " + psgeoSources);
}
...
function psgeoInitParameters() {
    ...
    // Settings are stored in the object psgeoSettings.
    // Variables:
    //
    // Methods:
    //     initDomain(hostname,pathname) -> a psgeoDomainDefault object

    const psgeoSettings = PsgeoSettings();
    psgeoDomainDefault = psgeoSettings.initDomain(hostname,pathname)


    psgeoInitDisplaySize();
    psgeoInitLanguage(hostname,urlParams);
    psgeoInitCavemaps(pathname,urlParams);
    psgeoInitLuolaseura(hostname,urlParams); // sets psgeoSources etc.
}
...
function psgeoInitMap() {
    ...
    var myPosition = {    // Lat, Lng. Updated in psgeoUpdateStatsPaneText() // XXXX Maybe should go to psgeoInitParameters() ??
        lat:null,
        lng:null,
        alt:null,
        heading:null,
    ...
    psgeoDebug("initList...");
    psgeoInitList(lib,lang,psgeoMap,myPosition);
