...
function psgeoHtmlFontSize(text,size) {
    return('<font size="' +
           size +
           '">' +
           text +
...
function psgeoOtherToolsMenuBringUp(db,lang,map) {
    ...
    if (psgeoOtherToolsWindow === undefined) {
        ...
        for (var i = 0; i < psgeoOtherTools.length; i++) {
            ...
            var columns = [
                psgeoHtmlLink(psgeoHtmlImage(tool.icon,'width="80"'),
                              tool.url,
                              true),
                psgeoHtmlFontSize(psgeoHtmlLink(lang[tool.nameFunction](),
                                                tool.url,
                                                true) +
                                  ": " +
