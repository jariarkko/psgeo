...
var psgeoDomainDefaults = [
    ...
    },
    {
        ...
        waterbodySearch: false,
	// Domain specific images imgXxxx for buttons					// RALF
	// 	to be used in psgeoGetXxxxMenuButton
	imgDefaultSize: 30,								// 30px is good default size	
	// imgFilterMenu: "./icons/neu_interface_filter_funnel_icon_149475.svg",	// Filtermenu
	...
	imgCaveClassification: "./icons/neu_interface_faq_question_icon_149479.svg",	// 	Help = Classification of caves
	...
	// More generic names suggested: imgFilter,imgInfo(<imgMore),
	// imgAbout,imgHelp(<imgCaveClassification),imgExternalLinks(<imgOtherTools),
	// imgAction(<imgCaveAdd)	// with generic names the info window could be generic
    },
    {
...
function psgeoFilterMenuBringUp(db,lib,lang,map,toOpen) {
    if (psgeoFilterWindow === undefined) {
        ...
        var div = document.createElement("div");
        var titleelement = document.createElement("h3");
        var titletext = document.createTextNode(lang.capitalize(lang.filterText()));
        titleelement.appendChild(titletext);
        div.appendChild(titleelement);
...
function psgeoGenericMenuButton(imgurl,lang,text,structure,fn) {	// RALF
...
function psgeoGetOtherToolsMenuButton(lang,structure,fn) {		// RALF
    return(psgeoGenericMenuButton(psgeoDomainDefault.imgOtherTools,lang,lang.moreMapsText(),structure,fn));
	}	
    
function psgeoGetFilterMenuButton(lang,structure,fn) {			// RALF
    return(psgeoGenericMenuButton(psgeoDomainDefault.imgFilter,lang,
                                  (psgeoSmallDisplayMode ? lang.filterText() : lang.moreFiltersText()),
                                  structure,
                                  fn));
}
    
function psgeoGetMoreMenuButton(lang,structure,fn) {			// RALF
    return(psgeoGenericMenuButton(psgeoDomainDefault.imgMore,lang,lang.toolsAndAboutText(),structure,fn));
}
    
function psgeoGetAboutMenuButton(lang,structure,fn) {			// RALF
    return(psgeoGenericMenuButton(psgeoDomainDefault.imgAbout,lang,lang.aboutText(),structure,fn));
}	

function psgeoGetCaveAddMenuButton(lang,structure,fn) {			// RALF
    return(psgeoGenericMenuButton(psgeoDomainDefault.imgCaveAdd,lang,					
...
function psgeoGetCaveClassificationMenuButton(lang,structure,fn) {	// RALF
    return(psgeoGenericMenuButton(psgeoDomainDefault.imgCaveClassification,lang,lang.caveClassificationText(),structure,fn));
}
...
function psgeoGetGenericCheckbox(id,text,structure,br) {
    ...
    if (br) label.appendChild(document.createElement("br"));
    structure.appendChild(label);
    return(box);
}
