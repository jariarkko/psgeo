...
function psgeoMarkerPopupItem(map,item,popup,db,lib,lang,style) {
    ...
    case "image":
        ...
        var width = (window.innerWidth * widthPercentage) / 100;
...
function psgeoGetGenericCheckbox(id,text,structure,initiallyChecked,br,nowrap) {
    // place checkbox + text inside a div (to control line wrapping)
    var box = document.createElement("input");
        box.type = "checkbox";
        box.value = id;
    ...
    if (nowrap === true ) {
        var div = document.createElement("div");	// UUSI
            div.appendChild(label);
            div.style.whiteSpace = "nowrap";	// UUSI
...
function psgeoGetSubtypeGroupCheckboxList(lib,lang,group,structure) {
    ...
    psgeoDebug("subtype group " + group.descriptionTextId +
               " arg was " + JSON.stringify(group.getSubActivitiesFunctionArgument));
    var result = [];
    var box;
    ...
    for (var k = 0; k < group.extraSubActivitiesBefore.length; k++) {
        ...
        // create "checkbox | ... | checkbox", linewrapped, if psgeoSmallDisplayMode=false;
	// OR one checkbox per line, if smallDisplayMode=true (->div.style.whiteSpace="nowrap")
        box = psgeoGetGenericCheckbox(subact,name,structure,true,false,psgeoSmallDisplayMode);
        result.push(box);
    }
    ...
    for (var i = 0; i < types.length; i++) {
        ...
        box = psgeoGetGenericCheckbox(subact,
                                      ...
                                      true,
                                      (i == types.length-1 &&
                                       group.extraSubActivitiesAfter.length == 0),psgeoSmallDisplayMode);
        result.push(box);
    }
    ...
    for (var j = 0; j < group.extraSubActivitiesAfter.length; j++) {
        ...
        box = psgeoGetGenericCheckbox(subact,
                                      ...
                                      structure,
                                      true,
                                      (j == group.extraSubActivitiesAfter.length-1),psgeoSmallDisplayMode);
        result.push(box);
    }
