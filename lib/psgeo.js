...
function psgeoFilterSectionPrefix(lang,text,subdiv) {
    ...
    // subdiv.appendChild(document.createElement("br")); // Filter group header on separate line?
}
...
function psgeoGetOnlyFinlandCheckbox(lib,lang,structure) {
    return(psgeoGetGenericCheckbox("only finland",lang.getText("onlyFinland",psgeoSmallDisplayMode),structure,true,false));
}

function psgeoGetOtherCountriesCheckbox(lib,lang,structure) {
    return(psgeoGetGenericCheckbox("other countries",lang.getText("otherCountries",psgeoSmallDisplayMode),structure,true,true));
}
