...
function psgeoUpdateStatsPaneText(db,
    ...
    if (psgeoGeolocationButton === undefined) {
        ...
        // INIT VARIABLES
        var geolocatorId = 0;          // Worker id needed to stop it. Actual real ids start from 1.
        ...
        // DEFINE AN INFOWINDOW WHICH, WHEN CLOSED, STOPS BACKGROUND WORKER
        var markerMyPosition = new google.maps.InfoWindow();
        if (psgeoDomainDefault.geolocationContinuousUpdate) {
            google.maps.event.addListener(markerMyPosition, 'closeclick', function() {
               ...
               // but ONLY if it exists (geolocationNotSupported case has marker but no worker).
               if (geolocatorId > 0) navigator.geolocation.clearWatch(geolocatorId);
	...
	function geolocationFailed() {
            markerMyPosition.setPosition(map.getCenter(lang.getText("geolocationFailed"))); // DEBUG
        }
            ...
            psgeoGetGeolocationButton(
                ...
                function() {
                    ...
                    if (!navigator.geolocation) {
                        markerMyPosition.setPosition(map.getCenter(psgeoDomainDefaults.geolocationNotSupported));
