...
function psgeoInitList(lib,lang,map,myPosition,psgeoDomainDefault) {
    ...
    // Read JSON files. Continue to psgeoFinishLoading only after the last read has completed!
    var jsonImportsRunning = Object.keys(psgeoDomainDefault.sources).length;
    psgeoDomainDefault.sources.forEach(function (source) { 
        $.getJSON(source.datafile,function(data) { // These run in parallel
            db.addPlacesWithFilter(data,db.getFilter(),source.textId);
            jsonImportsRunning--;
            psgeoDebug("Received data from "+source.datafile+" Sources remaining: "+jsonImportsRunning);
            if (jsonImportsRunning == 0) psgeoFinishLoading(db,lib,lang,map,filterForMarkers,myPosition,psgeoDomainDefault); 
        });
    });
}
...
function psgeoInitMap() {
    ...
    const settings = PsgeoSettings(); // new poistettu
