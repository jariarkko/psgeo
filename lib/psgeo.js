...
function psgeoRerunFilters(db,lib,lang,map,myPosition,psgeoDomainDefault) {
    ...
    for (i = 0; i < psgeoActivityFilterCheckboxes.length; i++) { // XXXXXXXXXX forEach !!!
    ...
    psgeoDebug("Rerun filters: before db.setFilter(finalFilter)");
        psgeoDebug("db has "+db.nPlaces()+" places."); // YYYYYYY
        psgeoDebug("Final filter is:"+finalFilter.toString());
...
function psgeoUpdateStatsPaneText(db,
    ...
    psgeoDebug("Statspane: db has "+db.nPlaces()+" places."); // YYYYYYYYY
