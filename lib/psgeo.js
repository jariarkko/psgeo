...
const psgeoRange = {
    measure: "length", // MUST correspond to a term in lang.
    unit: "meters",         // MUST correspond to a term in lang.
    limitHigh: 50,
    limitLow: 5
}
...
function psgeoInitFilterPaneText(db,lib,lang,map,div,smallPane,myPosition,psgeoDomainDefault) {
    ...
    if (psgeoDomainDefault.cavemaps) { // XXXXXXXXX CHANGE: cavemaps=true means that "show range filter"
        if (psgeoSmallDisplayMode) psgeoFilterSectionPrefix(lang,lang.getText("size",psgeoSmallDisplayMode),subsubdiv1b);

        // psgeoRange has: measure, unit, limitLow, limitHigh. lang has rangeHigh/Medium/Low texts with values.
        let measureName = lang.getText(psgeoRange.measure,false); // measure: "length", "depth", "grade", ... TRANSLATED
        let unit = lang.getText(psgeoRange.unit,false); // unit: "", "meters", "kilometers", "minutes", ... TRANSLATED
        psgeoCreateRangeCheckbox(db,
                                    ...
                                    lang.getTextWithValues("rangeHigh",
                                                           psgeoSmallDisplayMode,
                                                           [measureName,psgeoRange.limitHigh,unit]),
        ...
        psgeoCreateRangeCheckbox(db,
                                    ...
                                    lang.getTextWithValues("rangeMedium",
                                                           psgeoSmallDisplayMode,
                                                           [measureName,psgeoRange.limitLow,psgeoRange.limitHigh,unit]),
                                    !psgeoSmallDisplayMode)
        psgeoCreateRangeCheckbox(db,
                                    lang,
                                    document,
                                    ...
                                    lang.getTextWithValues("rangeLow",
                                                           psgeoSmallDisplayMode,
                                                           [measureName,psgeoRange.limitLow,unit]),
                                    !psgeoSmallDisplayMode)
...
function psgeoCreateOnlyFinlandFilter(db,lib,lang,map,psgeoDomainDefault) {
    ...
    function finlandPartFilter(checked,psgeoDomainDefault) { //XXXXXXXX  YKSINKERTAISTA
        if (checked) {
    ...
    function otherPartFilter(checked,psgeoDomainDefault) { // XXXXX YKSINKERTAISTA
        if (checked) {
    ...
    if (psgeoOnlyFinlandFilterCheckbox === undefined && psgeoOtherCountriesFilterCheckbox === undefined) { // XXXX YKSINKERTAISTA
        return(undefined);
...
function psgeoRerunFilters(db,lib,lang,map,myPosition,psgeoDomainDefault) {
    var newBasicFilters = [];
    var newSizeFilters = [];
    var newOnlyFinlandFilter = psgeoCreateOnlyFinlandFilter(db,lib,lang,map,psgeoDomainDefault); // YYYYYYYY
...
function psgeoInitList(lib,lang,map,myPosition,psgeoDomainDefault) {
    psgeoList = [];
    psgeoMarkerList = [];
    psgeoMarkerItemList = [];
    var db = PsgeoDB(lib,[],"");
    ...
    psgeoInitListOne(db,lib,lang,map,filterForMarkers,0,myPosition,psgeoDomainDefault);
}

function psgeoInitListOne(db,lib,lang,map,filterForMarkers,count,myPosition,psgeoDomainDefault) {
    if (count == psgeoSources.length) {
        ...
        psgeoDebug("Loading " + lang.getText(source.textId) +   // XXXXXX Should be English, but not critical
                   " from " + path +
                   "...");
        psgeoDebug("before getJSON filterForMarkers = " +
                   (filterForMarkers === undefined ? "ud" : (db.filterToString(filterForMarkers))));
        $.getJSON(path, function(data) {
            psgeoDebug("got data for " + lang.getText(source.textId)); // XXXXX Should be English, but not critical
            psgeoDebug("before addPlaces filterForMarkers = " +
                       (filterForMarkers === undefined ? "ud" : (db.filterToString(filterForMarkers))));
            psgeoDebug("adding places from " + lang.getText(source.textId) +    // XXXXXXX Again, English...
            ...
            psgeoInitListOne(db,lib,lang,map,filterForMarkers,count+1,myPosition,psgeoDomainDefault);
        });
    }
}
...
function psgeoFilterMenuContents(db,lib,lang,map,subdiv,myPosition,psgeoDomainDefault) {
    ...
    subdiv.addEventListener('click', function() {  // YYYYYYY
...
function psgeoGetGenericCheckbox(id,text,structure,initiallyChecked,br) {
    ...
    var box = document.createElement("input");
    box.type = "checkbox";
    box.value = id;			// XXXXX e.g. rangeHigh, rangeMedium, rangeLow
...
function psgeoInitParameters(hostname,pathname,urlParams,psgeoDomainDefault) {
    ...
    psgeoInitLuolaseura(hostname,urlParams,psgeoDomainDefault); // sets psgeoSources etc.
    return
}
...
function psgeoInitMap() {
    ...
    const settings = new PsgeoSettings();
    const psgeoDefaultIcons = settings.getDefaultIcons;
    const psgeoDomainDefault = settings.getDomainDefault(hostname,pathname)
    psgeoInitParameters(hostname,pathname,urlParams,psgeoDomainDefault);
    // psgeoInitDisplaySize();
    // psgeoInitLanguage(urlParams,psgeoDomainDefault);
    // psgeoInitCavemaps(pathname,urlParams,psgeoDomainDefault);
    // psgeoInitLuolaseura(hostname,urlParams,psgeoDomainDefault); // sets psgeoSources etc.
    ...
    psgeoInitPopup(psgeoMap);
        psgeoDebug("initList...");
    psgeoInitList(lib,lang,psgeoMap,myPosition,psgeoDomainDefault);
