...
var aboutExplanation0 = {
    "luolaseura": {
        "en": "This server employs two datasets of caves: one, the larger, by the Finnish Caving Association and Ralf Strandell. The second one is by the Planetcaver and Jari Arkko.",
        "fi": "Tällä palvelimella näytettävät tiedot tulevat kahdesta lähteestä. Pääosa tiedosta on Suomen Luolaseuran ja Ralf Strandellin tietokannasta. Tähän sisälty joidenkin Suomen Luolat kirjassa kerrottujen luolien tiedot. Lisäksi palvelimella näytetään Planetcaverin ja Jari Arkon vierailemia luolakohteita."
    },
    "other": {
...
var aboutExplanation1 = {
    "en": "The server is powered by Planet Skier Geo (Psgeo), an open source software that can draw activities on a map. It has filtering capabilities and is driven by a rich data representation about places, actions, and articles describing them. The tool is available at GitHub at https://github.com/jariarkko/psgeo. There's also a specification  for ActivityJSON, a specialisation of GeoJSON, to represent the rich data sets that are needed by the tool. The specification can be found at https://github.com/jariarkko/psgeo/blob/master/doc/activity-json.txt. The central component of Psgeo, PsgeoDB, is independently usable for searches, filtering and processing of activity-related data.",
    "fi": "Palvelin ajaa Psgeo-ohjelmistoa. Psgeo (Planet Skier Geo) on Javascript-kirjasto, jolla voidaan esittää aktiviteetteja ja harrastuspaikkoja kartalla. Ohjelmisto on avointa lähdekoodia, ja mahdollistaa tiedon filtteröinnin pohjauten alla olevaan semanttiseen tieton paikoista, aktiviteeteista, ja artikkeleista jotka kuvaavat niitä. Kirjasto löytyy GitHubista osoitteesta https://github.com/jariarkko/psgeo. Tiedonesityksen formaatti on myös määritelty, käytämme ActivityJSON-muotoisia JSON-tiedostoja. Tämä on GeoJSON-muotoisen tiedon jatkokehits. ActivityJSON:n määritelmä löytyy osoitteesta https://github.com/jariarkko/psgeo/blob/master/doc/activity-json.txt. Psgeo:n keskeinen osa, PsgeoDB, on myös käytettävissä aktiviteettitiedon käsittelyyn ja hakuihin."
}
...
var aboutExplanation3 = {
    "en": "<b>WARNINGS AND DISCLAIMERS</b>",
    "fi": "<b>VAROITUKSET JA VASTUUN RAJOITUKSET</b>"
}

var aboutExplanation4 = {
    "en": "It is important to recognise that Psgeo is a software library, and distinct from any data that is displayed with it. First off, map data is by Google and Karttapaikka, and the actual activity and location data comes from various sources.",
    "fi": "Psgeo pelkkä ohjelmistokirjasto, eikä sinänsä liity mitenkään tietoihin joita sen avulla esitetään. Karttatieto on ensinnäkin peräisin Googlen ja Karttapaikan palveluista, ja lisäksi varsinainen aktiviteetti ja paikkatieto on peräisin eri lähteistä."
}

var aboutExplanation5 = {
    "en": "Jari Arkko's Planetcaver and Planetskier provide one data set, of Jari's, Jarmo's, Janne's, Olli's, Tero's, and many other people's excursions. The Finnish Caving Association's web page has a large data set about caves in Finland and elsewhere. ",
    "fi": "Jari Arkon Planetcaver ja Planetskier-palvelimet käyttävät yhtä tietokantaa, jossa on Jarin, Jarmon, Janne, Ollin, Teron ja monien muiden kanssa tehtyjä retkiä. Suomen Luolaseuran sivustolla on toinen, huomattavasti laajempi tietokanta Suomalaista ja hieman muunkin maailman luolista."
};

var aboutExplanation6 = {
    "en": "The authors of Psgeo shall not be held responsible for any data displayed with it, or possible accidents resulting in attempting to go there. YOU HAVE BEEN WARNED! MOST LOCATIONS SHOWN WITH THIS TOOL ARE DANGEROUS! Similarly, the authors of various datasets about places are neither responsible for individual choices to visit the locations, and also obviously not responsible for what other people's data sets may contain.",
    "fi": "Psgeo:n ohjelmiston kirjoittajat eivät ole mitenkään vastuussa tiedosta, jota kirjastolla esitetään, tai onnettomuksista joita saattaa sattua, mikäli esitetyissä kohteissa vieraillaan. SINUA ON VAROITETTU, NÄYTETTÄVÄT PAIKAT OVAT YLEENSÄ VAARALLISIA! Samalla tavalla eri tietolähteiden tekijät eivät ole vastuussa tietoa katselevien käyttäjien toimista, eivätkä tietenkään muiden tietolähden ratkaisuista esittää jotain tietoa Psgeo:n kautta."
};
...
function psgeoAboutMenuBringUp(db,lang,map) {
    ...
    if (psgeoAboutWindow === undefined) {
        ...
        subdiv.innerHTML += psgeoHtmlParagraph(explanation);
        explanation = aboutExplanation3[lang.getLanguage()];
        subdiv.innerHTML += psgeoHtmlParagraph(explanation);
