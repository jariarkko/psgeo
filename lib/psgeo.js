...
function psgeoUpdateStatsText(db,
    ...
    if (psgeoMoreMenuButton === undefined) {
        psgeoMoreMenuButton = psgeoGetMoreMenuButton(lang,arrayitems[8]);
        psgeoMoreMenuButton.addEventListener('click', function() {
...
function psgeoMoreMenuContents(db,lib,lang,map,subdiv) {
    if (psgeoDomainDefault.sourcesCheckboxes) {
        if (psgeoSourceFilterCheckboxes.length == 0) {
            if (psgeoDomainDefault.luolaseura) {
                ...
                subdiv.appendChild(document.createElement("br"));
                psgeoSourceFilterCheckboxes = psgeoGetSourcesCheckboxes(subdiv);
            } else {
                subdiv.appendChild(document.createElement("br"));
                psgeoSourceFilterCheckboxes = psgeoGetSourcesCheckboxes(subdiv);
            }
    ...
    if (psgeoDomainDefault.luolaseura) {
        psgeoCaveAddMenuButton = psgeoGetCaveAddMenuButton(lang,subdiv);
        psgeoCaveAddMenuButton.addEventListener('click', function() {
	    psgeoDebug("Cave add button pressed!");
            psgeoCaveAddBringUp(db,lib,lang,map);
        });
        psgeoCaveClassificationMenuButton = psgeoGetCaveClassificationMenuButton(lang,subdiv);
        psgeoCaveClassificationMenuButton.addEventListener('click', function() {
	...
	psgeoDebug("other tools set up 1");
	    ...
	    psgeoDebug("other tools set up 2");
            psgeoOtherToolsMenuButton = psgeoGetOtherToolsMenuButton(lang,subdiv);
    ...
    if (psgeoAboutMenuButton === undefined) {
        psgeoAboutMenuButton = psgeoGetAboutMenuButton(lang,subdiv);
...
function psgeoGetMoreMenuButton(lang,structure) {
    psgeoDebug("more set up");
    var br = document.createElement("br");
    var label = document.createElement("button");
    var text = lang.capitalize(lang.moreText() + "...");
    label.innerHTML = text;
    label.style.background = "none";
    label.style.padding = 0;
    label.style.borderWith = 0;
...
function psgeoGetCaveAddMenuButton(lang,structure) {
    psgeoDebug("about set up");
    var br = document.createElement("br");
    var label = document.createElement("button");
    var text = lang.capitalize(lang.addText() +  " (" + lang.orModifyText() + ") "  + lang.caveText());
    label.innerHTML = text;
    label.style.background = "none";
    label.style.padding = 0;
    label.style.borderWith = 0;
...
function psgeoGetCaveClassificationMenuButton(lang,structure) {
    psgeoDebug("about set up");
    var br = document.createElement("br");
    var label = document.createElement("button");
    var text = lang.capitalize(lang.caveClassificationText());
    label.innerHTML = text;
    label.style.background = "none";
    label.style.padding = 0;
    label.style.borderWith = 0;
