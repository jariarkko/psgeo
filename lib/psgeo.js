    ...
    if (psgeoGeolocationButton === undefined) {
        ...
        if (psgeoDomainDefault.continuousGeolocation) {
            ...
            google.maps.event.addListener(markerMyPosition, 'closeclick', function() {
               ...
               psgeoDebug("User closed markerMyPosition. Disabling continuous geolocation. Worker stopped. geolocatorId was " + typeof(geolocatorId) + " and there have been " + geolocationClicks + " clicks.");
            });
        } else {
            markerMyPosition = new google.maps.InfoWindow();
        }
        ...
        function geolocationAchieved(position) {
            ...
            markerMyPosition.setContent(lang.getText("myGeolocation")+" Worker: "+geolocatorId); // DEBUG
	...
	function geolocationFailed() {
        ...
        psgeoGeolocationButton =
			...
			return;
                    ...
                    // STOP UPDATING POSITION? (every second click: 2,4,6,...) // DOES NOT WORK ON MOBILE
                    geolocationClicks++;
                    if ( geolocationClicks %2 === 0 ) { 
                        navigator.geolocation.clearWatch(geolocatorId);
                        markerMyPosition.close();
                        psgeoDebug("Click "+geolocationClicks+". Geolocation worker " + geolocatorId + " stopped. Marker removed.");
                        return;
                    }
