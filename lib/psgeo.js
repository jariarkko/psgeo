...
function psgeoInitMarker(item,filter) {
    ...
    if (psgeoCavemaps) {
        ...
        if (!hasUrl($.psgeoDB.getItemReadingList(item))) {
            t =
                $.psgeoDB.getItemName(item) +
                altstring +
                ":\n" +
                $.psgeoLang.noArticleYetText() +
                "";
        } else if ($.psgeoDB.getItemReadingList(item).length == 1) {
            t =
                $.psgeoDB.getItemName(item) +
                altstring +
                ":\n" +
                $.psgeoDB.getItemReadingList(item)[0].t;
        } else {
            t =
                $.psgeoDB.getItemName(item) +
                altstring +
                " (" +
                $.psgeoDB.getItemReadingList(item).length +
                " " +
	...
	title: t,
	url: psgeoCavemaps ? $.psgeoDB.getItemMap(item) : $.psgeoDB.getItemReadingList(item),
	icon: image,
...
function psgeoInitList() {
    ...
    for (i = 0; i < psgeoSources.length; i++) {
	    ...
	    $.psgeoDB.addPlacesWithFilter(data,$.psgeoDB.getFilter());
            count++;
            if (count == psgeoSources.length) {
                psgeoDebug("added places");
                var filterForMarkers = $.psgeoDB.getFilter();
                if (psgeoCavemaps) {
                    filterForMarkers =
