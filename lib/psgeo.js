...
function psgeoAboutMenuBringUp(db,lib,lang,map,psgeoDomainDefault) { 
    if (!psgeoDomainDefault.classicUI || psgeoAboutWindow === undefined) {
        ...
        let dataExplanation = "";
        let datasource;
        for (const dataSource of psgeoDomainDefault.sources) {
            if (datasource.descriptionTextId !== "" ) dataExplanation += lang.getText(descriptionTextId,psgeoSmallDisplayMode);
        }
        // Include a link, too.
        dataExplanation += " ";
