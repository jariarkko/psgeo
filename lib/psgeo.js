...
function psgeoCreateOnlyFinlandFilter(db,lib,lang,map) {
	...
	// HUU ONGELMA; ENTÄ JOS return(undefined?)
	// return(undefined);	// HUU
	if (psgeoOnlyFinlandFilterCheckbox === undefined) psgeoDebug("FIN UNDEF");
	if (psgeoOtherCountryFilterCheckbox === undefined) psgeoDebug("OTH UNDEF");

	psgeoDebug("KOHTA A");
    if (psgeoOnlyFinlandFilterCheckbox === undefined && psgeoOtherCountryFilterCheckbox === undefined) {
        return(undefined);
    }

	psgeoDebug("KOHTA B");
    if (psgeoOnlyFinlandFilterCheckbox !== undefined && psgeoOtherCountryFilterCheckbox === undefined) {
	psgeoDebug("FINLAND FILTER"); // HUU
        return(finlandPartFilter(psgeoOnlyFinlandFilterCheckbox.checked));
    }

	psgeoDebug("KOHTA C");
    if (psgeoOnlyFinlandFilterCheckbox === undefined && psgeoOtherCountryFilterCheckbox !== undefined) {
	psgeoDebug("OTHER COUNTRIES FILTER"); // HUU
        return(otherPartFilter(psgeoOtherCountryFilterCheckbox.checked));
    }
	psgeoDebug("KOHTA D");

    // ELSE
	psgeoDebug("FI AND OTHER"); // HUU
        var filters = [];
        var one = finlandPartFilter(psgeoOnlyFinlandFilterCheckbox.checked);
        if (one !== undefined) filters.push(one);
	psgeoDebug("KOHTA E");
        var other = otherPartFilter(psgeoOtherCountryFilterCheckbox.checked);
...
function psgeoFilterMenuContents(db,lib,lang,map,subdiv) {
    ...
    for (var i = 0; i < psgeoSubtypeGroups.length; i++) {
        ...
        psgeoDebug("considering subtype group " +
                   group.selector +
                   " " +
                   JSON.stringify(group.displaySizeSelector[psgeoSmallDisplayMode]));
    ...
    if (psgeoDomainDefault.finlandselection && !psgeoSmallDisplayMode) {
        ...
        psgeoOtherCountryFilterCheckbox = psgeoGetOtherCountryCheckbox(lib,lang,subdiv);
    }
...
function psgeoGetOnlyFinlandCheckbox(lib,lang,structure) {
    return(psgeoGetGenericCheckbox("only finland",lang.getText("onlyFinland"),structure,true,false));
}

function psgeoGetOtherCountryCheckbox(lib,lang,structure) {
    return(psgeoGetGenericCheckbox("other countries",lang.getText("otherCountries"),structure,true,true)); // HUU
}
