...
function psgeoMarkerClick(marker) {
    ...
    if (psgeoDomainDefault.cavemaps && map !== undefined) {
        ...
        if (rl.length == 1 &&
            ...
            var u = db.getReadingListItemURL(rl[0]);
            psgeoDebug("rl[0].u = " + JSON.stringify(u));
            if (u !== undefined) {
	        chosenUrl = u;
            }
        } else {
	    ...
	    chosenUrl = db.getReadingListItemURL(rl[choice]);
        }
    }
...
function psgeoArticlesText(db,item) {
    if (!db.hasItemReadingListUrl(item)) {
        return(psgeoCapitalize($.psgeoLang.noArticleYetText()));
    } else {
        var rl = db.getItemReadingList(item);
        if (rl.length == 1) {
            return(psgeoCapitalize($.psgeoLang.articleText()) +
                   ": " +
            ...
            return(psgeoCapitalize($.psgeoLang.articleText()) +
                   ...
                   $.psgeoLang.otherText() +
                   " " +
                   $.psgeoLang.articlesText());
        }
    }
}

function psgeoInitMarker(db,item,filter) {
    ...
    if (db.getItemFuzzy(item) != undefined && db.getItemFuzzy(item) === true) {
    ...
    if (psgeoDomainDefault.cavemaps) {
        ...
        if (db.getItemMap(item) !== undefined) {
            t += psgeoCapitalize($.psgeoLang.mapText());
            t += "\n";
        }
        var sidecaves =  db.getItemSideCaves(item);
        if (sidecaves.length > 0) {
            t += sidecaves.length.toString() + " " + $.psgeoLang.sideCavesText();
            t += "\n";
        }
        t += psgeoArticlesText(db,item);
    } else {
        t = db.getItemName(item);
        t += altstring;
        t += ":\n" + psgeoArticlesText(db,item);
    }
...
function psgeoNumberAlign(n,chs) {
    var ns = "" + n;
    while (ns.length < chs) {
	ns = " " + ns;
    }
...
function psgeoCreateCaveSizeCheckbox(db,document,subsubdiv1b,size,name) {
...
function psgeoCreateCaveMapCheckbox(db,document,subsubdiv1b,name) {
...
function psgeoCreateOnlyFinlandCheckbox(db,document,subsubdiv1b,name) {
...
function psgeoInitStats2Text(db,arrayitems) {
    var subdiv1 = arrayitems[0];
    var subdiv2 = dographs ? arrayitems[1] : null;
    
    var subsubdiv1b = document.createElement('div');
    var fif = db.filterMatchCountry("Finland");
    var notfif = db.filterNot(fif);
    var items = psgeoDomainDefault.cavemaps ? db.cities(fif).sort() : $.psgeoLib.getActivityList();
	...
	var item = items[i];
        if (psgeoDomainDefault.cavemaps) {
            psgeoDebug("does city " + item + " fit in with  "  +
                       ...
                       db.cityPlaceCount(item).toString());
    ...
    if (psgeoDomainDefault.cavemaps) {
        if (psgeoDomainDefault.cityselection) {
            if (otheritems.length > 0) {
                psgeoCreateCityOtherCheckbox(db,document,subsubdiv1b,otheritems,psgeoCapitalize($.psgeoLang.otherText()));
            }
...
function psgeoRerunFilters(db) {
    ...
    var sizeFilter = psgeoDomainDefault.cavemaps ? db.filterOr(newSizeFilters) : db.filterTrue();
...
function psgeoUpdateStatsText(arrayitems,ncontinents,ncountries,ncities,nstatesus,nstatesca,ncavemaps,nplaces) {
    ...
    if (psgeoDomainDefault.cavemaps && psgeoSmallDisplayMode) {
        ...
        arrayitems[0].innerHTML = (psgeoNumberAlign(nplaces,3) + " " + $.psgeoLang.placesText());
    }
    arrayitems[1].innerHTML = (psgeoNumberAlign(ncities,3) + " " + $.psgeoLang.citiesText());
    arrayitems[2].innerHTML = (psgeoNumberAlign(ncountries,3) + " " + $.psgeoLang.countriesText());
    if (!psgeoDomainDefault.cavemaps) {
        arrayitems[3].innerHTML = (psgeoNumberAlign(ncontinents,3) + " " + $.psgeoLang.continentsText());
        arrayitems[4].innerHTML = (psgeoNumberAlign(nstatesus,3) + " " + $.psgeoLang.statesText());
        arrayitems[5].innerHTML = (psgeoNumberAlign(nstatesca,3) + " " + $.psgeoLang.provincesText());
    }
    if (psgeoDomainDefault.cavemaps) {
        arrayitems[6].innerHTML = (psgeoNumberAlign(ncavemaps,3) + " " + $.psgeoLang.cavemapsText());
...
function psgeoUpdateStats(db) {
    ...
    psgeoUpdateStatsText(psgeoStatsTexts,
                         ncontinents,
                         ncountries,
                         ncities,
                         nstatesus,
...
function psgeoInitStats(db) {
    psgeoInitStats2Text(db,psgeoStats2Texts);
    psgeoUpdateStats(db);
}
    ...
    for (i = 0; i < psgeoSources.length; i++) {
	    ...
	    psgeoDebug("got data..., length = " + data.length.toString());
            psgeoDebug("before addPlaces filterForMarkers = " +
                       (filterForMarkers === undefined ? "ud" : (db.filterToString(filterForMarkers))));
	    db.addPlacesWithFilter(data,db.getFilter(),source.name[psgeoDomainDefault.lang]);
            count++;
            if (count == psgeoSources.length) {
                ...
                db.applyAllNoFilter(function(item) {
                    psgeoInitMarker(db,item,filterForMarkers);
                });
                psgeoDebug("initStats...");
                psgeoInitStats(db);
                psgeoDebug("CA states: " + db.states("Canada").toString());
