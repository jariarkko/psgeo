...
function psgeoInitFilterPaneText(db,lib,lang,map,div,smallPane,myPosition,tbl,psgeoDomainDefault) {
    ...
    div.addEventListener('click', function() {
        ...
        psgeoRerunFilters(db,lib,lang,map,myPosition,tbl,psgeoDomainDefault);
    });
...
function psgeoRerunFilters(db,lib,lang,map,myPosition,tbl,psgeoDomainDefault) {
    ...
    psgeoUpdateStatsPane(db,lib,lang,map,myPosition,tbl,psgeoDomainDefault);
}
        ...
        psgeoFilterMenuButton =
            psgeoGetFilterMenuButton(lang,
                                     ...
                                     function() {
                                         ...
                                         psgeoFilterMenuBringUp(db,lib,lang,map,true,psgeoSmallDisplayMode,myPosition,tbl,psgeoDomainDefault);
                                     },
                                     psgeoDomainDefault);
    }
...
function psgeoUpdateStatsPane(db,lib,lang,map,myPosition,tbl,psgeoDomainDefault) {
...
function psgeoInitStatsPane(db,lib,lang,map,myPosition,tbl,psgeoDomainDefault) {
    if (!psgeoSmallDisplayMode) {
        // create the 'small pane' filter window (upper window: length selector)
        psgeoInitFilterPaneText(db,lib,lang,map,psgeoFilterPaneTexts[0],true,myPosition,tbl,psgeoDomainDefault);
    }
    psgeoUpdateStatsPane(db,lib,lang,map,myPosition,tbl,psgeoDomainDefault);
}
...
function psgeoFinishLoading(db,lib,lang,map,filterForMarkers,myPosition,tbl,cfg,psgeoDomainDefault) {
    ...
    psgeoInitStatsPane(db,lib,lang,map,myPosition,tbl,psgeoDomainDefault);
    psgeoDebug("Loaded");
...
function psgeoFilterMenuContents(db,lib,lang,map,subdiv,myPosition,tbl,psgeoDomainDefault) {
    ...
    if (psgeoSmallDisplayMode) { // On small displays we only have the range filter on this popup window
        psgeoInitFilterPaneText(db,lib,lang,map,subdiv,false,myPosition,tbl,psgeoDomainDefault); 
    }
...
function psgeoFilterMenuBringUp(db,lib,lang,map,toOpen,psgeoSmallDisplayMode,myPosition,tbl,psgeoDomainDefault) {
...
if (!psgeoDomainDefault.classicUI && psgeoFilterWindow === undefined) {
    ...
    div.appendChild(document.createElement("br"));
    psgeoFilterMenuContents(db,lib,lang,map,subdiv,myPosition,tbl,psgeoDomainDefault);
...
function psgeoInitMap() {
    ...
    // All scripts have now loaded and DOM is ready
    // (as Google Maps was loaded with defer and this is the callback function)
    ...
    // Retrieve default values for global constants. Later the object will be passed on instead.
    //    Topographic map sources
    const psgeoFinnishTerrainMapBaseurl = cfg.psgeoFinnishTerrainMapBaseurl;
    //    Basic display settings 
    const psgeoDefaultZoomForPlace = cfg.psgeoDefaultZoomForPlace;
    const psgeoDefaultZoomForPlaceLatitudeShift = cfg.psgeoDefaultZoomForPlaceLatitudeShift;
    const psgeoSmallDisplayModeLimit = cfg.psgeoSmallDisplayModeLimit;
    const psgeoImagePercentageSmallDisplays = cfg.psgeoImagePercentageSmallDisplays;
    const psgeoImagePercentageLargeDisplays = cfg.psgeoImagePercentageLargeDisplays;
