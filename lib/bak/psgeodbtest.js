...
var min0filter = db4.filterRange("min",0);
var min5filter = db4.filterRange("min",5);
var max5filter = db4.filterRange("max",5);
var max100filter = db4.filterRange("max",100);
var min100filter = db4.filterRange("min",100);
db4.setFilter(min0filter);
f = db4.getFilter();
fs = db4.filterToString(f);
psgeoAssert("db4.filter",fs == "range>=0");
db4.setFilter(max5filter);
f = db4.getFilter();
fs = db4.filterToString(f);
psgeoAssert("db4.filter",fs == "range<5");
db4.setFilter(db4.filterAnd([min0filter,max5filter]));
f = db4.getFilter();
psgeoAssert("db4.filter",db4.filterToString(f) == "(range>=0 and range<5)");
var cs = db4.getFilterRanges(f);
...
f = db4.getFilter();
psgeoAssert("db4.filter",db4.filterToString(f) == "(range>=0 and range>=5 and range<100)");
var cs = db4.getFilterRanges(f);
debug("cs len " + cs.length.toString());
debug("cs min1 " + cs[0].min.toString() + " max1 " + cs[0].max.toString());
psgeoAssert("cs.length",cs.length == 1);
...
psgeoAssert("db4.filter",fs == "((range>=0 and range<5) or range>=100)");
var cs = db4.getFilterRanges(f);
debug("cs len " + cs.length.toString());
debug("cs min1 " + cs[0].min.toString() + " max1 " + cs[0].max.toString());
...
var psgeoRangeHigh = 50;
var psgeoRangeLow = 5;
debug("-- all ranges");
var csfilttest1filt =
    csfilttest1db.filterOr([csfilttest1db.filterRange("min",psgeoRangeHigh),
                            csfilttest1db.filterAnd([csfilttest1db.filterRange("min",psgeoRangeLow),
                                                     csfilttest1db.filterRange("max",psgeoRangeHigh)]),
                            csfilttest1db.filterRange("max",psgeoRangeLow)]);
...
csfilttest1filt =
    csfilttest1db.filterOr([csfilttest1db.filterAnd([csfilttest1db.filterRange("min",psgeoRangeLow),
                                                     csfilttest1db.filterRange("max",psgeoRangeHigh)]),
                            csfilttest1db.filterRange("max",psgeoRangeLow)]);
csfilttest1db.applyAllExtraFilter(csfilttest1filt,
                                  function (x) {
...
mergeissue60db.applyAll(function (x) {
    ...
    debug(description);
    debug("alternative names = " + JSON.stringify(mergeissue60db.getItemAlternativeNames(x)));
});
...
debug("-- quick subsumption tests");
debug("");
debug("Test case basic Javascript");
var jsobj1 = { "foo": 1, "bar": 2 }
var jslist1 = [jsobj1];
var jslist2 = [jsobj1];
jsobj1.snaap = 3;
debug("jslist1 = " + JSON.stringify(jslist1));
debug("jslist2 = " + JSON.stringify(jslist2));

var myk = {
    "n": "Korkbergetin lohkareluola, Kirkkonummi",
    "lat": 60.162099, "lon": 24.452772,
    "an": ["Korkbergetin eli Korpbergetin luolat 1, Kirkkonummi","Korkbergetin eli Korpbergetin luolat 1, Kirkkonummi","Korkbergetin luolat, Kirkkonummi"],
    "d": {"en": "This boulder cave is the largest known (or mapped) cave in Finland at 300+ meters of mazy passages. The cave is wedged between a cliff and the lake Humaljärvi.", "fi": "Tämä lohkareluola on ehkä suurin luola Suomessa, tai ainakin sen sokkeloisen verkoston pituus on pidempi kuin muissa kartoitetuissa suomalaisissa luolissa. Luola on n. 25 metrisen kallionjyrkänteen ja Humaljärven välissä. Luolasta löytyy tarkka kartta."},
    "m": "https://www.arkko.com/korkberget/korkberget-full.pdf", "l": 302.6, "la": "exact",
    "k": "Kirkkonummi", "c": "Finland", "o": "Europe",
    "rl": [{ "t": "The Korkberget Files",
             "i": "https://1.bp.blogspot.com/-NMrOC6EwS50/WFclsRezEnI/AAAAAAAAdCE/_ElORlkHmwAC_Vb_pVKRKWRD5TlGNwakQCLcB/s2048/blog227_pic_dragon.jpg",
             "r": 1,
             "y": 2016, "m": 12,
             "w": ["Jari Arkko","Janne Arkko","Jarmo Ruuth","Eetu Ruuth","Eino Ruuth","Taina Talvitie","Dare Talvitie","Tor Paulin","Velma Aho"],
             "a": ["Caving"],
             "sa": [["Rock","Rock-Granite","Morphology-Boulders","Basic","Boating","Surveying"]],
             "v": "https://vimeo.com/197046265",
             "u": "https://planetskier.blogspot.fi/2016/12/the-korkberget-files.html" },
           { "t": "A Pile of Rocks",
             "i": "https://4.bp.blogspot.com/-O9SDYcByemM/Voa5kYATs-I/AAAAAAAAcDI/MYYKNkUMlsY/s2048/blog192_peek.jpg",
             "y": 2015, "m": 12, "w": ["Jari Arkko","Janne Arkko","Jarmo Ruuth","Eetu Ruuth","Eino Ruuth"],
             "a": ["Caving"], "sa": [["Rock","Rock-Granite","Morphology-Boulders","Basic"]],
             "v": "https://vimeo.com/150548167", "u": "https://planetskier.blogspot.com/2016/01/a-pile-of-rocks.html" },
           { "t": "More cave photo & flash exercise",
             "i": "https://4.bp.blogspot.com/-BejklGjQdws/W9Hmk4CWv9I/AAAAAAAA6vs/wkN_3h3cz7cKPr472jDm-gGYlN45IaGpACLcBGAs/s2048/blog521_tunnel.jpg",
             "y": 2018, "m": 10,
             "w": ["Jari Arkko","Jukka Palm","Jarmo Ruuth"],
             "a": ["Caving"], "sa": [["Rock","Rock-Granite","Morphology-Boulders","Basic"]],
             "u": "https://planetskier.blogspot.com/2018/10/more-cave-photo-flash-exercise.html" },
           { "t": "Virtual caving club meeting. In a cave.",
             "i": "https://1.bp.blogspot.com/-TTyNG720AmM/Xpq7V-b3G_I/AAAAAAABdc0/2Sx8EAumwCoeGPMTuU6ErIVkAfB3WRUdQCLcBGAsYHQ/s2048/blog760_meeting.jpg",
             "y": 2020, "m": 4,
             "w": ["Jari Arkko","Jarmo Ruuth","Ralf Strandell","Velma Aho","Miri Simey","Jukka Palm","Johanna Raulio","Karoliina Artjoki","Dare Talvitie"],
             "a": ["Caving"], "sa": [["Rock","Rock-Granite","Morphology-Boulders","Social"]],
             "u": "https://planetskier.blogspot.com/2020/04/virtual-caving-club-meeting-in-cave.html" }]};
var lsk = {
    "n": "Korkbergetin luolat, Kirkkonummi",
    "lat": 60.161749740, "lon": 24.452431084,
    "an": ["Korkbergetin luolat", "Korpbergetin luolat"],
    "d": {"fi": "Luolan tunniste U22. Geneettis-morfologinen luokitus {E}.  Valtava lohkaresokkelo.",
          "se": "Finsk grott-ID U22. Finsk genetisk-morfologisk grottklass {E}. Mera information på finska.",
          "en": "Finnish cave ID U22. Finnish genetic-morphological cave class {E}. More information in Finnish." },
    "la": "exact",
    "l": 302.6,
    "k": "Uudenmaan lääni",
    "c": "Finland",
...
function qstest(name,lists) {
    debug("");
    debug("Test case " + name + ":");
    var qsdb = PsgeoDB(lib,lists[0],name);
    qsdb.applyAll(function (x) { debug(name + ": " + qsdb.describeItem(lang,x,false,false)); });
}
qstest("QS1",[qstest1list1]);
qstest("QS2",[qstest2list1]);
qstest("QS3",[qstest3list1,qstest3list2]);
qstest("QS4",[qstest3list1,qstest3list2]);

var bug265testdb = [
    {"n": "Pećina Beli Kop, Sremski Karlovci", "lat": 45.136540, "lon": 19.857720,
     "d": {"fi": "Pieni luola hiekkakivisessä kallionjyrkänteessä, lähellä luostaria.",
       "en": "A small cave under a sandstone cliff. There's a monastery nearby."},
     "m": "https://www.arkko.com/serbia-caves/Pecina-Beli-Kop.pdf",
     "d3": "https://www.arkko.com/serbia-caves/Pecina-Beli-Kop-3D-BLEND.blend",
     "ad3": ["https://www.arkko.com/serbia-caves/Pecina-Beli-Kop-3D-MP4.mp4","https://www.arkko.com/serbia-caves/Pecina-Beli-Kop-3D-JPG.png",
         "https://www.arkko.com/serbia-caves/Pecina-Beli-Kop-3D-ICON.jpg","https://www.arkko.com/serbia-caves/Pecina-Beli-Kop-3D-GLB.glb",
         "https://www.arkko.com/serbia-caves/Pecina-Beli-Kop-3D-STL.STL"],
     "l": 25.8, "la": "exact", "k": "Sremski Karlovci", "c": "Serbia", "o": "Europe",
     "rl": [{ "t": "Pećina Beli Kop",
          "i": "https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEgQrp6IPkqNaCOZuwKfOrhpEvu7Wvs8XDYosNUGBPu4BntIfjuNah9WxJZ2haHMd54i6ksPh1x10wbsWLq6WkS0tmd9syDi7zxcYWX-OPSdWCjcJ1mcBC2fLwI-Ykk-ceKyx2zfqTK681KrhjnV1U_PIt9XXz1VqfLGxSpoLrVrr-fbSmNzZyj6chIt9A/s4032/blog1077_cavefar2.jpg",
          "y": 2022, "m": 12, "w": ["Jari Arkko"], "a": ["Caving"], "sa": [["Basic","Rock","Rock-Sandstone","Morphology-Erosion","Surveying"]],
          "u": "https://planetskier.blogspot.com/2023/01/pecina-beli-kop.html" },
        { "t": "Goo",
          "a": [], "sa": [], "w": [],
          "u": "https://goo.gl/maps/h9RNdkXyf1z2JaHx6" }]}
];

function bug265testcb(a,b,c) {
    debug("bug265 callback " + a + "," + b + "," + c);
}

function bug265test() {
    debug("");
    debug("Test case for issue 265");
    var bug265db = PsgeoDB(lib,bug265testdb,"bug265db",bug265testcb);
    bug265db.setFilter(bug265db.filterTrue());
    var n = bug265db.nPlaces();
    psgeoAssert("bug265 nPlaces 1",n == 1);
    n = bug265db.nArticles();
    psgeoAssert("bug265 nArticles 1",n == 2);
    bug265db.setFilter(bug265db.filterFalse());
    n = bug265db.nPlaces();
    psgeoAssert("bug265 nPlaces 2",n == 0);
    bug265db.setFilter(lib.getDefaultFilterCaving(bug265db));
    n = bug265db.nPlaces();
    psgeoAssert("bug265 nPlaces 3",n == 1);
}
