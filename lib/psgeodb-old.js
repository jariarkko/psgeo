...
function PsgeoDB(lib,listin,source) {
    ...
    function normalizeItemFields(place,source) {
		...
		moretext += lang.getTextWithValues("CaveTypeIs",[type);	// HUU
                moretexten += " Cave type is " + type + ".";	// HUU
                moretextfi += " Luolan tyyppi on " + type + "."; // HUU
		...
		var prefix = "";	// HUU	
                ...
                if (lib.isdigit(lkdesc[0])) {
                    prefixen = langen.getText("dimensions") + " "; // HUU
                    prefixfi = langfi.getText("dimensions") + " "; // HUU
                }
                moretexten += " " + langen.capitalize(prefixen + lkdesc) + "."; // HUU
                moretextfi += " " + langfi.capitalize(prefixfi + lkdesc) + "."; // HUU
            }
            addDescriptions(place,
                            { d: { en: langen.getText("literatureCaveId") + " " + id + "." + moretexten, // HUU
                                   fi: langfi.getText("literatureCaveId") + " " + id + "." + moretextfi }, // HUU
                              s: source },
                            source);
        }
